...
dense-break-performance-pack-smoke output="artifacts/audio_qa/local-dense-break-performance-pack-smoke":
    ...
    python3 scripts/generate_dense_break_performance_pack.py --validate-report "{{output}}/performance-report.json"
    jq -e '.schema == "riotbox.dense_break_performance_pack.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .evidence_boundary.quality_proof == false and .source_policy.source_aware == true and (.source_policy.scripted_boundaries | length) >= 4 and (.source_policy.decisions.pressure_shape | type == "string") and .source_policy.pressure_lift_policy.source_aware == true and .source_policy.pressure_lift_policy.source_family == "dense_break" and .source_policy.pressure_lift_policy.lift_shape == "transient-pressure slam" and .source_policy.arrangement_policy.source_aware == true and .source_policy.arrangement_policy.role_order_source_derived == true and .source_policy.arrangement_policy.source_family == "dense_break" and .source_policy.arrangement_policy.candidate_count >= 6 and .source_policy.arrangement_policy.scripted_role_distance >= 1 and (.source_policy.arrangement_policy.role_order_signature | type == "string") and (.files.rebuild_only_performance | endswith(".wav")) and .proof.source_policy_decision_count >= 8 and .proof.pressure_lift_policy_decision_count >= 12 and .proof.arrangement_policy_decision_count >= 8 and .proof.arrangement_role_order_source_derived == 1.0 and .proof.arrangement_role_candidate_count >= 6.0 and .proof.arrangement_scripted_role_distance >= 1.0 and .proof.arrangement_pressure_role_count >= 2 and .proof.arrangement_destructive_role_count >= 2 and .proof.arrangement_failure_count == 0 and .proof.pressure_lift_bar5_to_bar4_rms_ratio >= 1.02 and .proof.w30_to_source_rms_ratio >= 0.22 and .proof.pressure_low_band_lift_ratio >= 1.12 and .proof.dropout_to_stutter_rms_ratio <= 0.18 and .proof.restore_to_hook_transient_ratio >= 0.85 and .proof.full_to_source_rms_ratio >= 0.78 and .proof.hook_to_source_transient_ratio >= 0.48 and .proof.pressure_to_hook_rms_ratio >= 1.30 and .proof.restore_to_pressure_rms_ratio >= 1.12 and .proof.rebuild_only_to_full_rms_ratio >= 0.42 and .proof.rebuild_only_to_source_rms_ratio >= 0.30 and .proof.rebuild_only_to_source_correlation <= 0.92 and .proof.source_on_to_rebuild_only_correlation <= 0.995 and .proof.rebuild_only_pressure_to_hook_rms_ratio >= 1.30 and .proof.rebuild_only_restore_to_pressure_rms_ratio >= 1.08 and .metrics.rebuild_only_performance.rms > 0.001 and .metrics.rebuild_only_performance.peak_abs <= 0.985' "{{output}}/performance-report.json"
    jq -e '.source_policy.mix_treatment_policy.source_aware == true and .source_policy.mix_treatment_policy.candidate_count >= 6 and .source_policy.mix_treatment_policy.fixed_treatment_distance >= 0.08 and .proof.mix_treatment_source_derived == 1.0 and .proof.mix_treatment_candidate_count >= 6.0 and .proof.mix_treatment_fixed_distance >= 0.08 and .proof.mix_treatment_output_contrast_ratio >= 2.10' "{{output}}/performance-report.json"
    jq -e '.source_policy.tail_shape_policy.source_aware == true and .source_policy.tail_shape_policy.candidate_count >= 6 and .source_policy.tail_shape_policy.fixed_tail_distance >= 0.20 and .proof.tail_shape_source_derived == 1.0 and .proof.tail_shape_candidate_count >= 6.0 and .proof.tail_shape_fixed_distance >= 0.20 and .proof.tail_shape_output_contrast_ratio >= 3.00' "{{output}}/performance-report.json"
    jq -e '.proof.hook_chop_source_character_score_floor >= 0.60 and .proof.hook_chop_source_character_score_span >= 0.10' "{{output}}/performance-report.json"
    jq -e '(.proof.strongest_audible_element as $e | ["kick","snare","bass","stab","silence","restore"] | index($e)) and .proof.strongest_audible_element_score >= 1.00 and .proof.strongest_audible_element_margin >= 0.05 and .proof.strongest_audible_element_candidate_count >= 5.0 and .proof.strongest_audible_element_ambiguous == 0.0' "{{output}}/performance-report.json"
    jq -e '.source_policy.pressure_lift_policy.tr909_drive >= 1.10 and .source_policy.pressure_lift_policy.break_snap_drive >= 1.20 and .proof.strongest_audible_element == "snare" and .proof.strongest_audible_element_snare_score >= 1.93 and .proof.dense_break_snare_pressure_margin >= 0.22 and .proof.dense_break_physical_drum_pressure_score >= 1.58 and .proof.dense_break_pressure_transient_to_hook_ratio >= 0.70' "{{output}}/performance-report.json"
    jq -e '.proof.rebuild_only_source_spectral_similarity >= 0.60 and .proof.rebuild_only_source_transient_retention >= 0.45 and .proof.rebuild_only_source_character_survival_score >= 0.70' "{{output}}/performance-report.json"
    jq -e '.proof.destructive_gesture_source_derived == 1.0 and .proof.destructive_static_distance_frames >= 256.0 and .proof.destructive_offset_distance_frames >= 512.0' "{{output}}/performance-report.json"
    tmp="$(mktemp)" && jq '.metrics.rebuild_only_performance.rms = 0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected silent rebuild-only dense-break report to fail" >&2; exit 1; fi && grep -q "rebuild_only_performance_too_quiet_or_silent" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.hook_chop_selection_source_derived = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected non-source-derived dense hook/chop report to fail" >&2; exit 1; fi && grep -q "hook_chop_selection_not_source_derived" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.hook_chop_source_character_score_floor = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected weak source-character selection report to fail" >&2; exit 1; fi && grep -q "hook_chop_source_character_too_weak" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.arrangement_role_order_source_derived = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected non-source-derived dense arrangement report to fail" >&2; exit 1; fi && grep -q "arrangement_role_order_not_source_derived" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.mix_treatment_source_derived = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected non-source-derived dense mix-treatment report to fail" >&2; exit 1; fi && grep -q "mix_treatment_not_source_derived" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.tail_shape_source_derived = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected non-source-derived dense tail-shape report to fail" >&2; exit 1; fi && grep -q "tail_shape_not_source_derived" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.tail_shape_fixed_distance = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected fixed dense tail-shape report to fail" >&2; exit 1; fi && grep -q "tail_shape_collapsed_to_fixed_recipe" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.strongest_audible_element = "none"' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing strongest-element dense report to fail" >&2; exit 1; fi && grep -q "strongest_audible_element_missing" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.strongest_audible_element_margin = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected ambiguous strongest-element dense report to fail" >&2; exit 1; fi && grep -q "strongest_audible_element_ambiguous" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.strongest_audible_element = "bass" | .proof.dense_break_physical_drum_pressure_score = 0.0 | .proof.dense_break_pressure_transient_to_hook_ratio = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected weak dense drum-pressure report to fail" >&2; exit 1; fi && grep -q "dense_break_snare_not_strongest" "$tmp.out" && grep -q "dense_break_physical_drum_pressure_too_weak" "$tmp.out" && grep -q "dense_break_pressure_transient_too_soft" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.rebuild_only_source_character_survival_score = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected source-character-lost dense report to fail" >&2; exit 1; fi && grep -q "rebuild_only_source_character_not_surviving" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.destructive_gesture_source_derived = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected non-source-derived dense destructive report to fail" >&2; exit 1; fi && grep -q "destructive_gesture_not_source_derived" "$tmp.out" && rm "$tmp" "$tmp.out"
...
agent-musical-review-pack-smoke output="artifacts/audio_qa/local-agent-musical-review-pack-smoke":
    ...
    jq -e '.schema == "riotbox.agent_musical_review_pack.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .source_recognition == "source_transformed_but_present" and (.visual_files.full_performance.waveform | endswith(".png")) and (.visual_files.full_performance.spectrogram | endswith(".png"))' "{{output}}/agent-review.json"
    for role in source_window chop_hook pressure_lift dropout_stutter restore_hit full_performance rebuild_only_performance; do test -s "{{output}}/visuals/$role.waveform.png"; test -s "{{output}}/visuals/$role.spectrogram.png"; done
...
professional-source-wav-pack-smoke output="artifacts/audio_qa/local-professional-source-wav-pack":
    ...
    jq -e '.schema == "riotbox.professional_source_wav_pack.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .case_count == 2 and .passed_case_count == 2 and ([.cases[].source_family] | sort == ["sparse_bass_pressure", "tonal_hook"]) and all(.cases[]; .result == "pass" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .pressure_lift_policy.source_aware == true and .pressure_lift_policy.source_family == .source_family and .arrangement_policy.source_aware == true and .arrangement_policy.source_family == .source_family and (.arrangement_policy.role_order_signature | type == "string") and .proof.pressure_lift_policy_decision_count >= 12 and .proof.arrangement_policy_decision_count >= 8 and .proof.arrangement_pressure_role_count >= 2 and .proof.arrangement_destructive_role_count >= 2 and .proof.arrangement_failure_count == 0 and .proof.pressure_lift_bar5_to_bar4_rms_ratio >= 1.02 and .proof.rebuild_only_to_full_rms_ratio >= 0.42 and .proof.rebuild_only_to_source_rms_ratio >= 0.30 and .proof.rebuild_only_to_source_correlation <= 0.92 and .proof.source_on_to_rebuild_only_correlation <= 0.995 and .metrics.full_performance_peak_abs <= 0.985 and .metrics.rebuild_only_performance_peak_abs <= 0.985 and (.audio_files.full_performance | endswith(".wav")) and (.audio_files.rebuild_only_performance | endswith(".wav")))' "{{output}}/professional-source-wav-pack.json"
    jq -e 'all(.cases[]; .proof.arrangement_role_order_source_derived == 1.0 and .proof.arrangement_role_candidate_count >= 6.0 and .proof.arrangement_scripted_role_distance >= 1.0)' "{{output}}/professional-source-wav-pack.json"
    jq -e 'all(.cases[]; .proof.mix_treatment_source_derived == 1.0 and .proof.mix_treatment_candidate_count >= 6.0 and .proof.mix_treatment_fixed_distance >= 0.08 and .proof.mix_treatment_output_contrast_ratio >= 2.10)' "{{output}}/professional-source-wav-pack.json"
    jq -e 'all(.cases[]; .proof.tail_shape_source_derived == 1.0 and .proof.tail_shape_candidate_count >= 6.0 and .proof.tail_shape_fixed_distance >= 0.20 and .proof.tail_shape_output_contrast_ratio >= 3.00)' "{{output}}/professional-source-wav-pack.json"
    jq -e 'all(.cases[]; (.proof.strongest_audible_element as $e | ["kick","snare","bass","stab","silence","restore"] | index($e)) and .proof.strongest_audible_element_score >= 1.00 and .proof.strongest_audible_element_margin >= 0.05 and .proof.strongest_audible_element_candidate_count >= 5.0)' "{{output}}/professional-source-wav-pack.json"
    jq -e 'all(.cases[]; .proof.rebuild_only_source_spectral_similarity >= 0.60 and .proof.rebuild_only_source_transient_retention >= 0.45 and .proof.rebuild_only_source_character_survival_score >= 0.70)' "{{output}}/professional-source-wav-pack.json"
    jq -e 'any(.cases[]; .source_family == "tonal_hook" and .proof.hook_chop_selection_source_derived == 1.0 and .proof.hook_chop_static_distance_frames >= 256.0 and .proof.hook_chop_offset_distance_frames >= 512.0)' "{{output}}/professional-source-wav-pack.json"
    jq -e 'any(.cases[]; .source_family == "tonal_hook" and .proof.w30_to_source_rms_ratio >= 0.22)' "{{output}}/professional-source-wav-pack.json"
    jq -e 'any(.cases[]; .source_family == "tonal_hook" and .proof.hook_chop_source_character_score_floor >= 0.60 and .proof.hook_chop_source_character_score_span >= 0.10)' "{{output}}/professional-source-wav-pack.json"
    jq -e 'any(.cases[]; .source_family == "tonal_hook" and .proof.destructive_gesture_source_derived == 1.0 and .proof.destructive_static_distance_frames >= 256.0 and .proof.destructive_offset_distance_frames >= 512.0)' "{{output}}/professional-source-wav-pack.json"
    tmp="$(mktemp)" && jq '.proof.hook_chop_selection_source_derived = 0.0' "{{output}}/tonal_rusharp_120/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected non-source-derived tonal hook/chop report to fail" >&2; exit 1; fi && grep -q "hook_chop_selection_not_source_derived" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.hook_chop_static_distance_frames = 0.0' "{{output}}/tonal_rusharp_120/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected static tonal hook/chop report to fail" >&2; exit 1; fi && grep -q "hook_chop_selection_collapsed_to_static_first_bar" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.hook_chop_riff_unique_source_offset_count = 1.0' "{{output}}/tonal_rusharp_120/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected narrow tonal hook/chop riff offsets to fail" >&2; exit 1; fi && grep -q "hook_chop_riff_source_offsets_too_narrow" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.destructive_static_distance_frames = 0.0' "{{output}}/tonal_rusharp_120/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected static tonal destructive report to fail" >&2; exit 1; fi && grep -q "destructive_gesture_collapsed_to_fixed_choice" "$tmp.out" && rm "$tmp" "$tmp.out"
    jq -e 'any(.cases[]; .source_family == "sparse_bass_pressure" and .proof.bass_movement_source_derived == 1.0 and .proof.sparse_bass_movement_static_distance_hz >= 1.25 and .proof.sparse_bass_movement_frequency_span_hz >= 8.0 and .proof.pressure_low_band_lift_ratio >= 1.60 and .proof.strongest_audible_element == "bass" and .proof.strongest_audible_element_margin >= 0.08)' "{{output}}/professional-source-wav-pack.json"
    tmp="$(mktemp)" && jq '.proof.bass_movement_source_derived = 0.0' "{{output}}/sparse_kicksnr_120/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected non-source-derived sparse bass movement report to fail" >&2; exit 1; fi && grep -q "sparse_bass_movement_not_source_derived" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.sparse_bass_movement_static_distance_hz = 0.0' "{{output}}/sparse_kicksnr_120/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected fixed sparse bass movement report to fail" >&2; exit 1; fi && grep -q "sparse_bass_movement_collapsed_to_fixed_contour" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.pressure_low_band_lift_ratio = 0.0' "{{output}}/sparse_kicksnr_120/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected weak sparse low-band pressure report to fail" >&2; exit 1; fi && grep -q "sparse_pressure_lift_lacks_low_band_support" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.proof.strongest_audible_element_margin = 0.0' "{{output}}/sparse_kicksnr_120/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected ambiguous sparse bass dominance report to fail" >&2; exit 1; fi && grep -q "sparse_bass_dominance_margin_too_low" "$tmp.out" && rm "$tmp" "$tmp.out"
    for case in tonal_rusharp_120 sparse_kicksnr_120; do test -s "{{output}}/$case/05_full_performance.wav"; test -s "{{output}}/$case/06_rebuild_only_performance.wav"; test -s "{{output}}/$case/performance-report.json"; done
...
edge-source-professional-diagnostics-smoke output="artifacts/audio_qa/local-edge-source-professional-diagnostics":
    ...
    python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "{{output}}/edge-source-professional-diagnostics.json"
    jq -e '.schema == "riotbox.edge_source_professional_diagnostics.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .automated_musical_approval == false and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .case_count == 2 and .weak_routed_case_count == 2 and ([.cases[].source_family] | sort == ["bad_timing", "pad_noise"]) and all(.cases[]; .musical_risk_status == "weak_routed" and .human_verdict == "unverified" and .automated_musical_approval == false and .quality_proof == false and (.rendered_audio_sha256 | length == 64) and (.source_window_sha256 | length == 64) and .rendered_audio_sha256 != .source_window_sha256 and .metrics.full_performance_rms > 0.01 and .proof.source_to_performance_correlation < 0.975 and (.proposed_fix_categories | length >= 1)) and any(.cases[]; .source_family == "pad_noise" and .source_timing.confidence_result == "degraded" and .source_timing.grid_use == "unavailable" and .pressure_lift_policy.source_family == "pad_noise" and .proof.pad_noise_texture_source_derived == 1.0 and .proof.pad_noise_texture_candidate_count >= 3.0 and .proof.pad_noise_texture_gate_static_distance_frames >= 256.0 and .proof.pad_noise_texture_stab_static_distance_frames >= 256.0 and .proof.pad_noise_texture_gate_stab_distance_frames >= 512.0 and .proof.pad_noise_texture_transient_ratio >= 0.72 and (.weak_output_signals | index("pad_noise_policy_path")) and (.weak_output_signals | index("pad_noise_texture_gate")) and (.proposed_fix_categories | index("source_selection"))) and any(.cases[]; .source_family == "bad_timing" and .source_timing.confidence_result == "candidate_ambiguous" and .source_timing.grid_use == "manual_confirm_only" and .pressure_lift_policy.source_family == "bad_timing" and .timing_policy.policy == "manual_confirm_cautious_arrangement" and .timing_policy.bar_locked_policy_allowed == false and .timing_policy.requires_user_confirmation == true and .proof.manual_confirm_cue_transient_score >= 0.030 and (.weak_output_signals | index("bad_timing_cautious_arrangement_path")) and (.weak_output_signals | index("confirm_timing_before_bar_locked_moves")) and (.weak_output_signals | index("ambiguous_downbeat")) and (.proposed_fix_categories | index("ui_cue")))' "{{output}}/edge-source-professional-diagnostics.json"
    jq -e 'all(.cases[]; (.proof.strongest_audible_element as $e | ["kick","snare","bass","stab","silence","restore"] | index($e)) and .proof.strongest_audible_element_score >= 1.00 and .proof.strongest_audible_element_margin >= 0.05 and .proof.strongest_audible_element_candidate_count >= 5.0)' "{{output}}/edge-source-professional-diagnostics.json"
    jq -e 'all(.cases[]; .proof.rebuild_only_source_spectral_similarity >= 0.60 and .proof.rebuild_only_source_transient_retention >= 0.45 and .proof.rebuild_only_source_character_survival_score >= 0.70)' "{{output}}/edge-source-professional-diagnostics.json"
    tmp="$(mktemp)" && jq '.cases[0].metrics.full_performance_rms = 0' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected silent edge-source diagnostic fixture to fail" >&2; exit 1; fi && grep -q "rendered_audio_silent" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[0].rendered_audio_sha256 = .cases[0].source_window_sha256' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected identical edge-source diagnostic fixture to fail" >&2; exit 1; fi && grep -q "identical_output" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq 'del(.cases[0].source_family)' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing source-family edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "missing_source_family_metadata" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[0].pressure_lift_policy.source_family = "dense_break"' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing pad/noise policy edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "pad_noise_policy_not_applied" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[0].proof.pad_noise_texture_source_derived = 0.0' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected non-source-derived pad/noise texture edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "pad_noise_texture_not_source_derived" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[0].proof.pad_noise_texture_gate_static_distance_frames = 0.0' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected collapsed pad/noise texture gate edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "pad_noise_texture_gate_collapsed_to_fixed_choice" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[0].proof.strongest_audible_element = "none"' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing strongest-element edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "strongest_audible_element_missing" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[0].proof.rebuild_only_source_character_survival_score = 0.0' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected source-character-lost edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "rebuild_only_source_character_not_surviving" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[1].pressure_lift_policy.source_family = "sparse_bass_pressure"' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing bad-timing cautious policy edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "bad_timing_cautious_policy_not_applied" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[1].timing_policy.bar_locked_policy_allowed = true' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected bar-locked bad-timing policy edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "bad_timing_bar_locked_policy_allowed" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[1].proof.manual_confirm_cue_transient_score = 0.0' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected weak bad-timing confirmation cue edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "bad_timing_confirmation_cue_too_weak" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.cases[0].proof.source_to_performance_correlation = 0.999' "{{output}}/edge-source-professional-diagnostics.json" > "$tmp" && if python3 scripts/generate_edge_source_professional_diagnostics.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected fallback-collapse edge diagnostic fixture to fail" >&2; exit 1; fi && grep -q "fallback_or_identical_output_collapse" "$tmp.out" && rm "$tmp" "$tmp.out"
    for case in pad_noise_fadapad_120 bad_timing_beat20_128; do test -s "{{output}}/$case/render/05_full_performance.wav"; test -s "{{output}}/$case/source-timing.json"; done
...
non-dense-professional-proof-pack-smoke output="artifacts/audio_qa/local-non-dense-professional-proof-pack" source_wav="artifacts/audio_qa/local-professional-source-wav-pack":
    ...
    jq -e '.schema == "riotbox.non_dense_professional_proof_pack.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .case_count == 2 and .passed_case_count == 2 and ([.cases[].source_family] | sort == ["sparse_bass_pressure", "tonal_hook"]) and all(.cases[]; .result == "pass" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .quality_proof == false and (.rendered_audio_sha256 | length == 64) and (.source_family_manifest_sha256 | length == 64) and (.source_family_validator_sha256 | length == 64) and (.review_prompt_sha256 | length == 64))' "{{output}}/non-dense-professional-proof-pack.json"
    for case in tonal_rusharp_120 sparse_kicksnr_120; do test -s "{{output}}/$case/source-family-validator.json"; test -s "{{output}}/$case/review-prompt.md"; done
...
professional-output-listening-pack-smoke output="artifacts/audio_qa/local-professional-output-listening-pack":
    ...
    jq -e '.schema == "riotbox.professional_output_listening_pack.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .evidence_role == "listening_review_scaffold" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .case_count == 3 and ([.cases[].source_family] | sort == ["dense_break", "sparse_bass_pressure", "tonal_hook"]) and all(.cases[]; .human_verdict == "unverified" and .demo_readiness == "unverified" and (.demo_worthy_reason | startswith("Worth review:")) and (.not_demo_worthy_reason | startswith("Not demo-ready yet:")) and .evidence_role == "listening_review_scaffold" and .quality_proof == false and (.candidate | endswith(".wav")) and (.candidate_sha256 | length == 64) and (.review_sha256 | length == 64))' "{{output}}/professional-output-listening-pack.json"
    for case in dense_beat03_130 tonal_rusharp_120 sparse_kicksnr_120; do test -s "{{output}}/reviews/$case/review.json"; test -s "{{output}}/reviews/$case/prompt.md"; jq -e '.demo_readiness == "unverified" and (.demo_worthy_reason | startswith("Worth review:")) and (.not_demo_worthy_reason | startswith("Not demo-ready yet:"))' "{{output}}/reviews/$case/review.json"; grep -q "Demo Readiness" "{{output}}/reviews/$case/prompt.md"; done
...
professional-output-suite-smoke output="artifacts/audio_qa/local-professional-output-suite":
    ...
    tmp="$(mktemp)" && jq '(.children[] | select(.id == "dense_break").key_metrics.w30_to_source_rms_ratio) = 0.0' "{{output}}/professional-output-suite.json" > "$tmp" && if python3 scripts/validate_professional_output_suite_contract.py "$tmp" --output "{{output}}" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected weak hook/chop W30 suite contract to fail" >&2; exit 1; fi && grep -q "dense_hook_chop_w30_too_weak" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '(.children[] | select(.id == "professional_source_wav_pack").key_metrics.sparse_bass_dominance_margin) = 0.0' "{{output}}/professional-output-suite.json" > "$tmp" && if python3 scripts/validate_professional_output_suite_contract.py "$tmp" --output "{{output}}" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected weak sparse bass dominance suite contract to fail" >&2; exit 1; fi && grep -q "source_wav_sparse_bass_dominance_margin_too_low" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '(.children[] | select(.id == "destructive_variation").key_metrics.stutter_to_hook_transient_ratio) = 0.0' "{{output}}/professional-output-suite.json" > "$tmp" && if python3 scripts/validate_professional_output_suite_contract.py "$tmp" --output "{{output}}" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected weak destructive stutter suite contract to fail" >&2; exit 1; fi && grep -q "destructive_stutter_lacks_transient_impact" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.feral_mix_balance.max_support_generated_to_source_rms_ratio = 0.99' "{{output}}/professional-output-suite.json" > "$tmp" && if python3 scripts/validate_professional_output_suite_contract.py "$tmp" --output "{{output}}" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected masking generated-support suite contract to fail" >&2; exit 1; fi && grep -q "feral_generated_support_masks_source" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '(.children[] | select(.id == "dense_break").key_metrics.dense_break_pressure_transient_to_hook_ratio) = 0.0' "{{output}}/professional-output-suite.json" > "$tmp" && if python3 scripts/validate_professional_output_suite_contract.py "$tmp" --output "{{output}}" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected soft dense drum-transient suite contract to fail" >&2; exit 1; fi && grep -q "dense_pressure_transient_too_soft" "$tmp.out" && rm "$tmp" "$tmp.out"
...
sound-quality-readiness-report-smoke output="artifacts/audio_qa/local-sound-quality-readiness-report":
    ...
    jq -e '.schema == "riotbox.sound_quality_readiness_report.v1" and .result == "pass" and .phase == "P023" and .release_readiness == "blocked" and .quality_claim_allowed == false and .weak_output_routing.available == true and .weak_output_routing.production_fix_candidate_count >= 5 and .weak_output_routing.production_fix_summary.candidate_count == .weak_output_routing.production_fix_candidate_count and (.weak_output_routing.production_fix_summary.recurring_fix_categories | index("chop_policy")) and (.weak_output_routing.production_fix_summary.recurring_fix_categories | index("destructive_gesture")) and (.weak_output_routing.production_fix_candidates[] | select(.category == "chop_policy" and .software_next_step and .musician_payoff)) and (.weak_output_routing.fix_categories | index("chop_policy")) and (.weak_output_routing.fix_categories | index("bass_movement")) and (.professional_output_suite.source_character_selection.dense_hook_chop_score_floor >= 0.60) and (.professional_output_suite.source_character_selection.tonal_hook_chop_score_floor >= 0.60) and (.professional_output_suite.source_character_selection.min_rebuild_only_source_character_survival_score >= 0.70) and (.professional_output_suite.drum_pressure.dense_strongest_audible_element == "snare") and (.professional_output_suite.drum_pressure.dense_break_physical_drum_pressure_score >= 1.58) and (.professional_output_suite.drum_pressure.dense_break_pressure_transient_to_hook_ratio >= 0.70) and (.professional_output_suite.mix_balance.result == "pass") and ((["bass","snare","stab"] - .professional_output_suite.strongest_audible_elements) | length == 0) and (.human_review_queue.available == true) and (.human_review_queue.review_queue_count >= 5) and (.human_review_queue.high_priority_count >= 3) and (.human_review_queue.medium_priority_count >= 1) and (.human_review_queue.source_families | index("pad_noise")) and (.human_review_queue.source_families | index("weak_source")) and (.human_review_queue.source_families | index("bad_timing")) and (.human_review_queue.review_blockers | index("human_verdict_unverified")) and any(.human_review_queue.candidates[]; .entry_id == "pad-noise-fadapad-unverified-candidate" and .review_priority == "high" and (.strongest_audible_element | test("texture|gate|silence")) and (.source_character | test("pad/noise|texture")) and (.required_verdict_current_state == "human_verdict:unverified/demo_readiness:unverified") and .required_listening_question_count >= 6) and any(.human_review_queue.candidates[]; .entry_id == "sparse-bass-pressure-updated-unverified-candidate" and .review_priority == "medium" and (.strongest_audible_element | test("bass|kick"))) and all(.human_review_queue.candidates[]; .human_verdict == "unverified" and .demo_readiness == "unverified" and .quality_claim == false and (.review_blockers | index("quality_claim_blocked"))) and (.source_family_coverage.missing_demo_candidate_families == []) and (.source_family_coverage.missing_human_verdict_families | index("pad_noise")) and (.source_family_coverage.missing_human_verdict_families | index("weak_source")) and (.source_family_coverage.missing_human_verdict_families | index("bad_timing")) and (.source_family_coverage.missing_demo_ready_families | index("sparse_drums")) and (.source_family_coverage.missing_demo_ready_families | index("pad_noise")) and (.source_family_coverage.missing_demo_ready_families | index("weak_source")) and (.source_family_coverage.missing_demo_ready_families | index("bad_timing")) and any(.source_family_coverage.families[]; .source_family == "sparse_drums" and .status == "human_verdict_non_demo") and any(.source_family_coverage.families[]; .source_family == "pad_noise" and .status == "candidate_only") and any(.source_family_coverage.families[]; .source_family == "weak_source" and .status == "candidate_only") and any(.source_family_coverage.families[]; .source_family == "bad_timing" and .status == "candidate_only") and (.demo_bank.unverified_candidate_ids | index("tonal-hook-rusharp-unverified-candidate")) and (.demo_bank.unverified_candidate_ids | index("pad-noise-fadapad-unverified-candidate")) and (.demo_bank.unverified_candidate_ids | index("sparse-bass-pressure-updated-unverified-candidate")) and (.demo_bank.unverified_candidate_ids | index("bad-timing-beat20-unverified-candidate")) and (.demo_bank.unverified_candidate_ids | index("weak-source-beat20-rejection-unverified-candidate")) and (.next_fix_categories | index("destructive_gesture")) and all(.blockers[]; .code != "source_family_demo_candidate_missing") and (.blockers[] | select(.code == "source_family_human_verdict_missing")) and (.blockers[] | select(.code == "source_family_demo_ready_coverage_missing")) and (.blockers[] | select(.code == "human_review_queue_unverified_candidates_present"))' "{{output}}/sound-quality-readiness-report.json"
...
tonal-hook-professional-fixtures:
    tmp="$(mktemp -d)" && python3 scripts/validate_tonal_hook_professional.py --json-output "$tmp/tonal-hook.json" --markdown-output "$tmp/tonal-hook.md" scripts/fixtures/automated_musical_fitness/valid_tonal_hook_chop/manifest.json && jq -e '.schema == "riotbox.tonal_hook_professional.v1" and .result == "pass" and .source_family == "tonal_hook" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .metrics.w30_contribution_ratio >= .thresholds.min_w30_contribution_ratio and .metrics.w30_unique_source_offset_count >= .thresholds.min_w30_unique_slice_offsets and .metrics.identity_correlation <= .thresholds.max_identity_correlation' "$tmp/tonal-hook.json" && grep -q "Tonal-Hook Professional" "$tmp/tonal-hook.md" && rm -rf "$tmp"
    tmp="$(mktemp -d)" && if python3 scripts/validate_tonal_hook_professional.py scripts/fixtures/tonal_hook_professional/invalid_hookless/manifest.json >"$tmp/invalid.out" 2>&1; then cat "$tmp/invalid.out" >&2; rm -rf "$tmp"; echo "expected hookless tonal fixture to fail" >&2; exit 1; fi && grep -q "w30_hook_not_dominant" "$tmp/invalid.out" && grep -q "source_copy_not_transformed_enough" "$tmp/invalid.out" && rm -rf "$tmp"
...
release-demo-human-review-queue-fixtures output="artifacts/audio_qa/local-release-demo-human-review-queue":
    ...
    python3 scripts/generate_release_demo_human_review_queue.py --validate-report "{{output}}/release-demo-human-review-queue.json"
    jq -e '.schema == "riotbox.release_demo_human_review_queue.v1" and .result == "pass" and .phase == "P023" and .source_files_required == false and .quality_claim_allowed == false and .review_queue_count >= 5 and any(.review_queue[]; .entry_id == "pad-noise-fadapad-unverified-candidate" and .review_priority == "high" and .source_family == "pad_noise" and (.source_character | test("pad/noise|texture")) and (.strongest_audible_element | test("texture|gate|silence")) and (.demo_worthy_reason | test("playable")) and (.not_demo_ready_reason | test("human_verdict is unverified")) and (.review_blockers | index("source_family_human_verdict_missing")) and (.required_verdict_path.current_state == "human_verdict:unverified/demo_readiness:unverified") and (.required_listening_questions | length == 6)) and any(.review_queue[]; .entry_id == "bad-timing-beat20-unverified-candidate" and .review_priority == "high" and .source_family == "bad_timing" and (.source_character | test("Beat20")) and (.review_blockers | index("source_family_demo_ready_human_pass_missing"))) and any(.review_queue[]; .entry_id == "weak-source-beat20-rejection-unverified-candidate" and .review_priority == "high" and .source_family == "weak_source" and (.source_character | test("source-character failure")) and (.not_demo_ready_reason | test("quality claims are blocked"))) and any(.review_queue[]; .entry_id == "sparse-bass-pressure-updated-unverified-candidate" and .review_priority == "medium" and .source_family == "sparse_drums" and (.strongest_audible_element | test("bass|kick")) and (.review_blockers | index("source_family_demo_ready_human_pass_missing"))) and all(.review_queue[]; .human_verdict == "unverified" and .demo_readiness == "unverified" and .quality_claim == false and (.demo_worthy_reason | length > 20) and (.not_demo_ready_reason | test("unverified")) and (.required_verdict_path.release_ready_blocker | test("must not claim quality"))) and (.source_family_coverage.family_gaps[] | select(.source_family == "weak_source" and .missing_human_verdict == true and .missing_demo_ready_human_pass == true)) and (.source_family_coverage.family_gaps[] | select(.source_family == "sparse_drums" and .missing_human_verdict == false and .missing_demo_ready_human_pass == true)) and (.next_actions[] | select(.category == "listening_pack"))' "{{output}}/release-demo-human-review-queue.json"
    ...
    tmp="$(mktemp)" && jq '.review_queue[0].quality_claim = true' "{{output}}/release-demo-human-review-queue.json" > "$tmp" && if python3 scripts/generate_release_demo_human_review_queue.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected review queue quality claim to fail" >&2; exit 1; fi && grep -q "claims_quality" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.review_queue[0].source_character = ""' "{{output}}/release-demo-human-review-queue.json" > "$tmp" && if python3 scripts/generate_release_demo_human_review_queue.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected review queue missing source character to fail" >&2; exit 1; fi && grep -q "source_character_missing" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.review_queue[0].required_verdict_path.current_state = "human_verdict:pass/demo_readiness:demo_ready"' "{{output}}/release-demo-human-review-queue.json" > "$tmp" && if python3 scripts/generate_release_demo_human_review_queue.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected review queue stale verdict state to fail" >&2; exit 1; fi && grep -q "stale_verdict_state" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq 'del(.review_queue[0].required_listening_questions[2])' "{{output}}/release-demo-human-review-queue.json" > "$tmp" && if python3 scripts/generate_release_demo_human_review_queue.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected review queue incomplete listening questions to fail" >&2; exit 1; fi && grep -q "required_listening_questions_invalid" "$tmp.out" && rm "$tmp" "$tmp.out"
