...
source-family-release-demo-coverage-fixtures output="artifacts/audio_qa/local-source-family-release-demo-coverage":
    ...
    tmp="$(mktemp -d)" && jq -n '{schema:"riotbox.listening_review.v1",schema_version:1,human_verdict:"reject",reviewer:"Owner"}' >"$tmp/review.json" && review_hash="$(sha256sum "$tmp/review.json" | cut -d ' ' -f1)" && jq --arg review "$tmp/review.json" --arg review_hash "$review_hash" '.evidence_role = "live_review" | (.entries[] | select(.entry_id == "bad-timing-beat20-unverified-candidate")) |= (.human_verdict = "fail" | .demo_readiness = "not_demo_ready" | .fix_categories = ["source_selection"] | .human_review_evidence = {schema:"riotbox.demo_bank_human_review_evidence.v1",reviewer:"Owner",reviewer_kind:"human",review_path:$review,review_sha256:$review_hash} | .degraded_or_reject_evidence = {schema:"riotbox.demo_bank_degraded_or_reject_review_evidence.v1",outcome:"reject",product_path_reviewed:true,fallback_music_present:false,reason:"Timing stayed untrusted, so the live product rejected confident bar-locked output."})' scripts/fixtures/release_grade_demo_bank/demo_bank_v1.json >"$tmp/live-bank.json" && python3 scripts/validate_source_family_release_demo_coverage.py --evidence-mode live_readiness --demo-bank "$tmp/live-bank.json" --json-output "$tmp/live-generic-review.json" && jq -e '(.missing_family_success_families | index("bad_timing")) and any(.families[]; .source_family == "bad_timing" and .status == "human_verdict_non_demo" and (.degraded_or_reject_entry_ids | length) == 0)' "$tmp/live-generic-review.json" && rm -rf "$tmp"
