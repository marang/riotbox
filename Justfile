...
sound-quality-readiness-report-smoke output="artifacts/audio_qa/local-sound-quality-readiness-report":
    ...
    jq -e '.schema == "riotbox.sound_quality_readiness_report.v1" and .result == "pass" and .phase == "P023" and .release_readiness == "blocked" and .quality_claim_allowed == false and .weak_output_routing.available == true and .weak_output_routing.production_fix_candidate_count >= 5 and (.weak_output_routing.production_fix_candidates[] | select(.category == "chop_policy" and .software_next_step and .musician_payoff)) and (.weak_output_routing.fix_categories | index("chop_policy")) and (.weak_output_routing.fix_categories | index("bass_movement")) and (.source_family_coverage.missing_demo_candidate_families | index("pad_noise")) and (.source_family_coverage.missing_human_verdict_families | index("bad_timing")) and (.source_family_coverage.missing_demo_ready_families | index("sparse_drums")) and (.source_family_coverage.missing_demo_ready_families | index("pad_noise")) and any(.source_family_coverage.families[]; .source_family == "sparse_drums" and .status == "human_verdict_non_demo") and (.demo_bank.unverified_candidate_ids | index("tonal-hook-rusharp-unverified-candidate")) and (.next_fix_categories | index("destructive_gesture")) and (.blockers[] | select(.code == "source_family_demo_candidate_missing")) and (.blockers[] | select(.code == "source_family_human_verdict_missing")) and (.blockers[] | select(.code == "source_family_demo_ready_coverage_missing"))' "{{output}}/sound-quality-readiness-report.json"
...
source-family-release-demo-coverage-fixtures output="artifacts/audio_qa/local-source-family-release-demo-coverage":
    ...
    jq -e '.schema == "riotbox.source_family_release_demo_coverage.v1" and .result == "pass" and .source_files_required == false and .release_readiness == "blocked" and .quality_claim_allowed == false and .required_family_count == 6 and .covered_demo_ready_family_count == 2 and (.missing_demo_candidate_families | index("pad_noise")) and (.missing_demo_candidate_families | index("weak_source")) and (.missing_demo_candidate_families | index("bad_timing")) and (.missing_human_verdict_families | index("pad_noise")) and (.missing_human_verdict_families | index("weak_source")) and (.missing_human_verdict_families | index("bad_timing")) and (.missing_demo_ready_families | index("sparse_drums")) and (.missing_demo_ready_families | index("pad_noise")) and (.blockers[] | select(.code == "source_family_demo_candidate_missing" and .source_family == "pad_noise")) and any(.families[]; .source_family == "dense_break" and .status == "demo_ready_covered" and (.demo_ready_entry_ids | index("dense-break-beat03-human-pass"))) and any(.families[]; .source_family == "sparse_drums" and .status == "human_verdict_non_demo" and (.human_verdict_entry_ids | index("sparse-bass-pressure-human-weak"))) and any(.families[]; .source_family == "pad_noise" and .status == "missing_candidate")' "{{output}}/source-family-release-demo-coverage.json"
    ...
    tmp="$(mktemp -d)" && jq '.entries += [{entry_id:"pad-noise-unverified-candidate",source_family:"tonal_pad",source_path:"data/test_audio/examples/DH_Fadapad_120_A.wav",rendered_wav:{path:"artifacts/audio_qa/release-demo-bank/pad-noise/05_full_performance.wav",sha256:"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},metrics:{path:"artifacts/audio_qa/release-demo-bank/pad-noise/performance-report.json",sha256:"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"},review_prompt:{path:"artifacts/audio_qa/release-demo-bank/pad-noise/prompt.md",sha256:"cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"},human_verdict:"unverified",demo_readiness:"unverified",quality_claim:false,demo_worthiness_note:"Pad/noise candidate exists but has no human verdict yet.",fix_categories:[],musical_summary:{hook_within_two_bars:"unverified",hardest_audible_element:"unverified",source_character:"unverified",destructive_contrast:"unverified",bass_drum_pressure:"unverified",live_triggerability:"unverified",eight_bar_replay_value:"unverified"}}]' scripts/fixtures/release_grade_demo_bank/demo_bank_v1.json >"$tmp/demo-bank.json" && python3 scripts/validate_source_family_release_demo_coverage.py --demo-bank "$tmp/demo-bank.json" --json-output "$tmp/coverage.json" && jq -e '.release_readiness == "blocked" and (.missing_demo_candidate_families | index("pad_noise") | not) and (.missing_human_verdict_families | index("pad_noise")) and any(.families[]; .source_family == "pad_noise" and .status == "candidate_only")' "$tmp/coverage.json" && rm -rf "$tmp"
