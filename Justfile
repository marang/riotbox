...
sound-quality-readiness-report-smoke output="artifacts/audio_qa/local-sound-quality-readiness-report":
    ...
    jq -e '.schema == "riotbox.sound_quality_readiness_report.v1" and .result == "pass" and .phase == "P023" and .release_readiness == "blocked" and .quality_claim_allowed == false and .weak_output_routing.available == true and .weak_output_routing.production_fix_candidate_count >= 5 and (.weak_output_routing.production_fix_candidates[] | select(.category == "chop_policy" and .software_next_step and .musician_payoff)) and (.weak_output_routing.fix_categories | index("chop_policy")) and (.weak_output_routing.fix_categories | index("bass_movement")) and (.source_family_coverage.missing_demo_candidate_families | index("weak_source")) and (.source_family_coverage.missing_human_verdict_families | index("pad_noise")) and (.source_family_coverage.missing_human_verdict_families | index("bad_timing")) and (.source_family_coverage.missing_demo_ready_families | index("sparse_drums")) and (.source_family_coverage.missing_demo_ready_families | index("pad_noise")) and any(.source_family_coverage.families[]; .source_family == "sparse_drums" and .status == "human_verdict_non_demo") and any(.source_family_coverage.families[]; .source_family == "pad_noise" and .status == "candidate_only") and (.demo_bank.unverified_candidate_ids | index("tonal-hook-rusharp-unverified-candidate")) and (.demo_bank.unverified_candidate_ids | index("pad-noise-fadapad-unverified-candidate")) and (.demo_bank.unverified_candidate_ids | index("sparse-bass-pressure-updated-unverified-candidate")) and (.next_fix_categories | index("destructive_gesture")) and (.blockers[] | select(.code == "source_family_demo_candidate_missing")) and (.blockers[] | select(.code == "source_family_human_verdict_missing")) and (.blockers[] | select(.code == "source_family_demo_ready_coverage_missing"))' "{{output}}/sound-quality-readiness-report.json"
...
release-grade-demo-bank-fixtures:
    tmp="$(mktemp)" && python3 scripts/validate_release_grade_demo_bank.py --json-output "$tmp" scripts/fixtures/release_grade_demo_bank/demo_bank_v1.json && jq -e '.schema == "riotbox.release_grade_demo_bank.v1" and .result == "pass" and .entry_count >= 8 and .demo_ready_count >= 2 and .verdict_counts.pass >= 2 and .verdict_counts.weak >= 2 and .verdict_counts.fail >= 1 and .verdict_counts.unverified >= 3 and (.source_families | index("dense_break")) and (.source_families | index("tonal_hook")) and (.source_families | index("sparse_bass_pressure")) and (.source_families | index("tonal_pad"))' "$tmp" && rm "$tmp"
...
source-family-release-demo-coverage-fixtures output="artifacts/audio_qa/local-source-family-release-demo-coverage":
    ...
    jq -e '.schema == "riotbox.source_family_release_demo_coverage.v1" and .result == "pass" and .source_files_required == false and .release_readiness == "blocked" and .quality_claim_allowed == false and .required_family_count == 6 and .covered_demo_ready_family_count == 2 and (.missing_demo_candidate_families | index("pad_noise") | not) and (.missing_demo_candidate_families | index("weak_source")) and (.missing_demo_candidate_families | index("bad_timing")) and (.missing_human_verdict_families | index("pad_noise")) and (.missing_human_verdict_families | index("weak_source")) and (.missing_human_verdict_families | index("bad_timing")) and (.missing_demo_ready_families | index("sparse_drums")) and (.missing_demo_ready_families | index("pad_noise")) and (.blockers[] | select(.code == "source_family_human_verdict_missing" and .source_family == "pad_noise")) and any(.families[]; .source_family == "dense_break" and .status == "demo_ready_covered" and (.demo_ready_entry_ids | index("dense-break-beat03-human-pass"))) and any(.families[]; .source_family == "sparse_drums" and .status == "human_verdict_non_demo" and (.human_verdict_entry_ids | index("sparse-bass-pressure-human-weak")) and (.candidate_entry_ids | index("sparse-bass-pressure-updated-unverified-candidate")) and (.unverified_entry_ids | index("sparse-bass-pressure-updated-unverified-candidate"))) and any(.families[]; .source_family == "pad_noise" and .status == "candidate_only" and (.candidate_entry_ids | index("pad-noise-fadapad-unverified-candidate")))' "{{output}}/source-family-release-demo-coverage.json"
