...
release-demo-human-review-queue-fixtures output="artifacts/audio_qa/local-release-demo-human-review-queue":
    ...
    jq -e '.schema == "riotbox.release_demo_human_review_queue.v1" and .result == "pass" and .phase == "P023" and .source_files_required == false and .quality_claim_allowed == false and .review_queue_count >= 5 and any(.review_queue[]; .entry_id == "pad-noise-fadapad-unverified-candidate" and .review_priority == "high" and .source_family == "pad_noise") and any(.review_queue[]; .entry_id == "bad-timing-beat20-unverified-candidate" and .review_priority == "high" and .source_family == "bad_timing") and any(.review_queue[]; .entry_id == "weak-source-beat20-rejection-unverified-candidate" and .review_priority == "high" and .source_family == "weak_source") and any(.review_queue[]; .entry_id == "sparse-bass-pressure-updated-unverified-candidate" and .review_priority == "medium" and .source_family == "sparse_drums") and all(.review_queue[]; .human_verdict == "unverified" and .demo_readiness == "unverified" and .quality_claim == false) and (.source_family_coverage.family_gaps[] | select(.source_family == "weak_source" and .missing_human_verdict == true and .missing_demo_ready_human_pass == true)) and (.source_family_coverage.family_gaps[] | select(.source_family == "sparse_drums" and .missing_human_verdict == false and .missing_demo_ready_human_pass == true)) and (.next_actions[] | select(.category == "listening_pack"))' "{{output}}/release-demo-human-review-queue.json"
