...
sound-quality-readiness-report-smoke output="artifacts/audio_qa/local-sound-quality-readiness-report":
    ...
    tmp="$(mktemp)" && jq 'del(.ui_cue_priority)' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing current UI-cue priority detail to fail readiness validation" >&2; exit 1; fi && grep -q "ui_cue_priority_missing" "$tmp.out" && rm "$tmp" "$tmp.out"
