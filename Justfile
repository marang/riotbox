...
release-grade-demo-bank-fixtures:
    tmp="$(mktemp)" && python3 scripts/validate_release_grade_demo_bank.py --json-output "$tmp" scripts/fixtures/release_grade_demo_bank/demo_bank_v1.json && jq -e '.schema == "riotbox.release_grade_demo_bank.v1" and .result == "pass" and .entry_count >= 9 and .demo_ready_count >= 2 and .verdict_counts.pass >= 2 and .verdict_counts.weak >= 2 and .verdict_counts.fail >= 1 and .verdict_counts.unverified >= 4 and (.source_families | index("dense_break")) and (.source_families | index("tonal_hook")) and (.source_families | index("sparse_bass_pressure")) and (.source_families | index("tonal_pad")) and (.source_families | index("bad_timing"))' "$tmp" && rm "$tmp"
