...
professional-output-suite-smoke output="artifacts/audio_qa/local-professional-output-suite":
    ...
    jq -e '.schema == "riotbox.professional_output_suite.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .evidence_role == "suite_diagnostic" and .scripted_generation == true and .quality_proof == false and .child_report_count == 6 and .passed_child_report_count == 6 and .failed_child_report_count == 0 and (.children | length) == 6 and all(.children[]; .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .scripted_generation == true and .quality_proof == false and (.report_sha256 | length == 64)) and (.children | map(.id) | sort == ["dense_break", "destructive_variation", "pro_pressure_source_matrix", "professional_output_listening_pack", "professional_source_wav_pack", "rendered_weak_professional_outputs"]) and .listening_identity.result == "pass" and .listening_identity.case_count == 3 and (.listening_identity.source_families == ["dense_break", "sparse_bass_pressure", "tonal_hook"])' "{{output}}/professional-output-suite.json"
    test -s "{{output}}/README.md"; test -s "{{output}}/professional-output-listening-pack/reviews/dense_beat03_130/review.json"; test -s "{{output}}/destructive-variation/destructive-variation.json"; test -s "{{output}}/rendered-weak-professional-outputs/rendered-weak-professional-outputs.json"
