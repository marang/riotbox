...
listening-manifest-validate-generated-packs:
    tmpdir="$(mktemp -d)" && trap 'rm -rf "$tmpdir"' EXIT && python3 scripts/write_synthetic_break_wav.py "$tmpdir/source.wav" 4.0 && cargo run -p riotbox-audio --bin lane_recipe_pack -- --output-dir "$tmpdir/lane-recipe" --duration-seconds 2.0 && python3 scripts/validate_listening_manifest_json.py "$tmpdir/lane-recipe/manifest.json" && cargo run -p riotbox-audio --bin feral_before_after_pack -- --source "$tmpdir/source.wav" --output-dir "$tmpdir/feral-before-after" --duration-seconds 1.0 --source-window-seconds 0.5 && python3 scripts/validate_listening_manifest_json.py "$tmpdir/feral-before-after/manifest.json"
...
observer-audio-correlate-generated-feral-grid:
    tmpdir="$(mktemp -d)" && trap 'rm -rf "$tmpdir"' EXIT && python3 scripts/write_synthetic_break_wav.py "$tmpdir/source.wav" 4.0 && cargo run -p riotbox-audio --bin feral_grid_pack -- --source "$tmpdir/source.wav" --output-dir "$tmpdir/feral-grid" --bars 2 --source-window-seconds 0.5 && python3 scripts/validate_listening_manifest_json.py "$tmpdir/feral-grid/manifest.json" && cargo run -p riotbox-app --bin observer_audio_correlate -- --observer crates/riotbox-app/tests/fixtures/observer_audio_correlation/events.ndjson --manifest "$tmpdir/feral-grid/manifest.json" --require-evidence
