...
human-listening-label-corpus-fixtures:
    tmp="$(mktemp)" && python3 scripts/validate_human_listening_label_corpus.py --json-output "$tmp" scripts/fixtures/human_listening_label_corpus/valid_dense_break.json && jq -e '.schema == "riotbox.human_listening_label_corpus.v1" and .result == "pass" and .label_count == 3 and .verdict_counts.pass == 1 and .verdict_counts.weak == 1 and .verdict_counts.fail == 1 and (.source_families == ["dense_break"])' "$tmp" && rm "$tmp"
...
audio-judge-spike-fixtures:
    tmp="$(mktemp -d)" && python3 scripts/prototype_audio_judge_spike.py --agent-review scripts/fixtures/audio_judge_spike/local-agent-musical-review-pack-smoke/agent-review.json --agent-review scripts/fixtures/audio_judge_spike/local-agent-musical-review-pack-smoke-weak/agent-review.json --agent-review scripts/fixtures/audio_judge_spike/local-agent-musical-review-pack-smoke-fail/agent-review.json --label-corpus scripts/fixtures/human_listening_label_corpus/valid_dense_break.json --json-output "$tmp/audio-judge-spike.json" --markdown-output "$tmp/audio-judge-spike.md" && jq -e '.schema == "riotbox.audio_judge_spike.v1" and .result == "pass" and .judge_readiness == "not_ready" and .human_verdict == "unverified" and (.candidate_evaluations | length) == 3 and .metrics_baseline.predicted_label == "pass" and .calibration.label_count == 3 and .calibration.matched_label_count == 3 and (.calibration.matched_verdicts == ["fail", "pass", "weak"]) and (.calibration.failure_examples | length) == 0 and .calibration.confusion_matrix.pass.pass == 1 and .calibration.confusion_matrix.weak.weak == 1 and .calibration.confusion_matrix.fail.fail == 1 and (.providers | length) == 2' "$tmp/audio-judge-spike.json" && grep -q "Judge readiness" "$tmp/audio-judge-spike.md" && rm -rf "$tmp"
...
audio-judge-spike-generated-smoke output="artifacts/audio_qa/local-agent-musical-review-pack-smoke" report="artifacts/audio_qa/local-audio-judge-spike-generated-smoke":
    ...
    jq -e '.schema == "riotbox.audio_judge_spike.v1" and .result == "pass" and .judge_readiness == "not_ready" and .metrics_baseline.provider == "riotbox_metrics_baseline" and .calibration.matched_label_count == 1 and (.calibration.failure_examples | length) == 2' "{{report}}/audio-judge-spike.json"
