...
dense-break-performance-pack-smoke output="artifacts/audio_qa/local-dense-break-performance-pack-smoke":
    ...
    tmp="$(mktemp)" && jq '.source_policy.arrangement_policy.selection_strategy = "fallback-scripted-source-family-role-order"' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected fallback selection strategy dense-break report to fail" >&2; exit 1; fi && grep -q "fallback_selection_strategy:report.source_policy.arrangement_policy.selection_strategy" "$tmp.out" && rm "$tmp" "$tmp.out"
...
professional-output-listening-pack-smoke output="artifacts/audio_qa/local-professional-output-listening-pack":
    ...
    jq -e '.schema == "riotbox.professional_output_listening_pack.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .evidence_role == "listening_review_scaffold" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .case_count == 3 and .mc202_source_composed_review_gate.result == "pass" and .mc202_source_composed_review_gate.source_composed_case_count == 3 and .mc202_source_composed_review_gate.dense_break_case_count >= 1 and .mc202_source_composed_review_gate.non_dense_break_case_count >= 2 and .mc202_source_composed_review_gate.quality_proof == false and ([.cases[].source_family] | sort == ["dense_break", "sparse_bass_pressure", "tonal_hook"]) and all(.cases[]; .human_verdict == "unverified" and .demo_readiness == "unverified" and (.demo_worthy_reason | startswith("Worth review:")) and (.not_demo_worthy_reason | startswith("Not demo-ready yet:")) and .evidence_role == "listening_review_scaffold" and .quality_proof == false and (.candidate | endswith(".wav")) and (.candidate_sha256 | length == 64) and (.review_sha256 | length == 64) and .mc202_source_composed_review_gate.promotion_blocked_until_human_pass == true and .mc202_source_composed_review_gate.source_composed_evidence == true and .mc202_source_composed_review_gate.primitive_or_template_only == false) and any(.cases[]; .source_family == "dense_break" and .mc202_source_composed_review_gate.family_kind == "dense_break" and .mc202_source_composed_review_gate.source_composed_evidence == true) and any(.cases[]; .source_family == "sparse_bass_pressure" and .mc202_source_composed_review_gate.family_kind == "non_dense_break" and .mc202_source_composed_review_gate.source_composed_evidence == true and .mc202_source_composed_review_gate.metrics.bass_movement_source_derived == 1.0) and any(.cases[]; .source_family == "tonal_hook" and .mc202_source_composed_review_gate.family_kind == "non_dense_break" and .mc202_source_composed_review_gate.source_composed_evidence == true and .mc202_source_composed_review_gate.metrics.mc202_to_w30_rms_ratio >= 0.16)' "{{output}}/professional-output-listening-pack.json"
    for case in dense_beat03_130 tonal_rusharp_120 sparse_kicksnr_120; do test -s "{{output}}/reviews/$case/review.json"; test -s "{{output}}/reviews/$case/prompt.md"; jq -e '.demo_readiness == "unverified" and (.demo_worthy_reason | startswith("Worth review:")) and (.not_demo_worthy_reason | startswith("Not demo-ready yet:")) and .mc202_source_composed_review_gate.promotion_blocked_until_human_pass == true and .mc202_source_composed_review_gate.source_composed_evidence == true and .mc202_source_composed_review_gate.primitive_or_template_only == false and .audio_judge_label.mc202_source_composed_review_gate.source_composed_evidence == .mc202_source_composed_review_gate.source_composed_evidence' "{{output}}/reviews/$case/review.json"; grep -q "Demo Readiness" "{{output}}/reviews/$case/prompt.md"; done
