...
sound-quality-readiness-report-smoke output="artifacts/audio_qa/local-sound-quality-readiness-report":
    ...
    python3 scripts/generate_sound_quality_readiness_report.py --weak-routing "{{output}}/weak-output/weak-output-fix-routing.json" --perform-risk-cue-contract "{{output}}/jam-perform-risk-cue-contract/jam-perform-risk-cue-contract.json" --human-review-queue "{{output}}/human-review-queue/release-demo-human-review-queue.json" --release-demo-review-packs "{{output}}/release-demo-review-packs/release-demo-listening-review-packs.json" --output "{{output}}" --date "local-sound-quality-readiness-report"
    ...
    grep -q "Release-Demo Review Worklist" "{{output}}/sound-quality-readiness-report.md"
    grep -q "Generated review packs: \`5\`" "{{output}}/sound-quality-readiness-report.md"
    grep -q "Review pack: \`{{output}}/release-demo-review-packs/bad-timing-beat20-unverified-candidate\`" "{{output}}/sound-quality-readiness-report.md"
    grep -q "Pack prompt: \`{{output}}/release-demo-review-packs/bad-timing-beat20-unverified-candidate/prompt.md\`" "{{output}}/sound-quality-readiness-report.md"
    grep -q "Verdict target: record \`pass\`, \`weak\`, or \`fail\`" "{{output}}/sound-quality-readiness-report.md"
    grep -q "Listening questions:" "{{output}}/sound-quality-readiness-report.md"
    grep -q "Human Review Queue" "{{output}}/sound-quality-readiness-report.md"
    grep -q "Rendered WAV: \`artifacts/audio_qa/local-edge-source-professional-diagnostics/bad_timing_beat20_128/render/05_rebuild_only_performance.wav\`" "{{output}}/sound-quality-readiness-report.md"
    grep -q "Review prompt: \`artifacts/audio_qa/release-demo-bank/bad-timing-beat20/prompt.md\`" "{{output}}/sound-quality-readiness-report.md"
    tmp="$(mktemp)" && jq '.professional_output_suite.destructive_gesture.stutter_to_source_transient_ratio = 0 | .professional_output_suite.destructive_gesture.restore_to_source_transient_ratio = 0' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing source-referenced destructive proof to fail readiness validation" >&2; exit 1; fi && grep -q "current_evidence_reconciliation_destructive_gesture_stale_without_current_proof" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.professional_output_suite.source_selection_policy.promotion_allowed_source_families -= ["tonal_hook"]' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing tonal source-selection policy coverage to fail readiness validation" >&2; exit 1; fi && grep -q "current_evidence_reconciliation_source_selection_stale_without_candidate_family_coverage" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.professional_output_suite.drum_pressure.tr909_rendered_min_support_mix_contribution_ratio = 0' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing rendered TR-909 drum-pressure proof to fail readiness validation" >&2; exit 1; fi && grep -q "current_evidence_reconciliation_drum_pressure_stale_without_current_proof" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.jam_perform_risk_cue_contract.unavailable_action = "load source"' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected broken perform-risk cue contract to fail readiness validation" >&2; exit 1; fi && grep -q "current_evidence_reconciliation_ui_cue_stale_without_tui_contract" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '(.weak_output_routing.production_fix_candidates[] | select(.category == "fixture_threshold") | .primary_case_ids) = ["rendered_dense_flat_stutter"]' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected primary fixture-threshold candidate to fail stale reconciliation" >&2; exit 1; fi && grep -q "current_evidence_reconciliation_fixture_threshold_stale_without_negative_control_proof" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '.next_actions += [{category:"fixture_threshold", target:"weak output", action:"Keep stale fixture_threshold fixtures as regression controls."}]' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected stale weak-output next action to fail post-reconciliation readiness validation" >&2; exit 1; fi && grep -q "next_actions_stale_weak_output_controls_obscure_review_work" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '(.next_actions[] | select(.category == "source_selection" and .target == "bad_timing")) |= del(.candidate_id)' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing source-family review candidate context to fail readiness validation" >&2; exit 1; fi && grep -q "next_actions_bad_timing_review_candidate_context_missing" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq '(.next_actions[] | select(.category == "source_selection" and .target == "bad_timing")) |= del(.rendered_wav)' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing source-family review artifact ref to fail readiness validation" >&2; exit 1; fi && grep -q "next_actions_bad_timing_review_candidate_context_missing" "$tmp.out" && rm "$tmp" "$tmp.out"
    ...
    tmp="$(mktemp)" && jq '.release_demo_review_packs.packs[0].human_verdict = "pass"' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected verified review pack to fail readiness validation" >&2; exit 1; fi && grep -q "release_demo_review_pack_0_human_verdict_not_unverified" "$tmp.out" && rm "$tmp" "$tmp.out"
    tmp="$(mktemp)" && jq 'del(.release_demo_review_packs.packs[] | select(.entry_id == "bad-timing-beat20-unverified-candidate")) | .release_demo_review_packs.review_pack_count -= 1' "{{output}}/sound-quality-readiness-report.json" > "$tmp" && if python3 scripts/generate_sound_quality_readiness_report.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected missing candidate review pack to fail readiness validation" >&2; exit 1; fi && grep -q "release_demo_review_pack_bad-timing-beat20-unverified-candidate_candidate_context_missing" "$tmp.out" && rm "$tmp" "$tmp.out"
