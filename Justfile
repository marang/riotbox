...
pro-pressure-source-matrix-smoke output="artifacts/audio_qa/local-pro-pressure-source-matrix":
    ...
    jq -e '.schema == "riotbox.pro_pressure_source_matrix.v1" and .result == "pass" and .agent_verdict == "agent_promising" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .case_count >= 4 and .passed_case_count == .case_count and (.cases | length) == .case_count and ([.cases[].pressure_lift_policy.source_family] | unique | length) >= 2 and all(.cases[]; .result == "pass" and .human_verdict == "unverified" and .evidence_role == "diagnostic" and .source_backed == true and .source_timing_backed == true and .scripted_generation == true and .quality_proof == false and .pressure_lift_policy.source_aware == true and .proof.pressure_lift_policy_decision_count >= 12 and .proof.pressure_lift_bar5_to_bar4_rms_ratio >= 1.02 and .proof.w30_to_source_rms_ratio >= 0.18 and .proof.pressure_to_hook_rms_ratio >= 1.30 and .proof.restore_to_pressure_rms_ratio >= 1.12 and .metrics.full_performance_peak_abs <= 0.985)' "{{output}}/source-matrix-report.json"
