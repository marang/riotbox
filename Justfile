...
dense-break-performance-pack-smoke output="artifacts/audio_qa/local-dense-break-performance-pack-smoke":
    ...
    jq -e '.proof.hook_chop_source_character_score_floor >= 0.55 and .proof.hook_chop_source_character_score_span >= 0.10' "{{output}}/performance-report.json"
    ...
    tmp="$(mktemp)" && jq '.proof.strongest_audible_element = "bass" | .proof.dense_break_physical_drum_pressure_score = 0.0' "{{output}}/performance-report.json" > "$tmp" && if python3 scripts/generate_dense_break_performance_pack.py --validate-report "$tmp" >"$tmp.out" 2>&1; then cat "$tmp.out" >&2; rm "$tmp" "$tmp.out"; echo "expected weak dense drum-pressure report to fail" >&2; exit 1; fi && grep -q "dense_break_snare_not_strongest" "$tmp.out" && grep -q "dense_break_physical_drum_pressure_too_weak" "$tmp.out" && rm "$tmp" "$tmp.out"
...
pro-pressure-source-matrix-smoke output="artifacts/audio_qa/local-pro-pressure-source-matrix":
    ...
    jq -e 'any(.cases[]; .pressure_lift_policy.source_family == "dense_break" and .proof.hook_chop_source_character_score_floor >= 0.55 and .proof.hook_chop_source_character_score_span >= 0.10)' "{{output}}/source-matrix-report.json"
...
professional-source-wav-pack-smoke output="artifacts/audio_qa/local-professional-source-wav-pack":
    ...
    jq -e 'any(.cases[]; .source_family == "tonal_hook" and .proof.hook_chop_source_character_score_floor >= 0.55 and .proof.hook_chop_source_character_score_span >= 0.10)' "{{output}}/professional-source-wav-pack.json"
