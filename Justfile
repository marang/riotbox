...
sound-quality-readiness-report-smoke output="artifacts/audio_qa/local-sound-quality-readiness-report":
    ...
    jq -e '.schema == "riotbox.sound_quality_readiness_report.v1" and .result == "pass" and .phase == "P023" and .release_readiness == "blocked" and .quality_claim_allowed == false and .weak_output_routing.available == true and .weak_output_routing.production_fix_candidate_count >= 5 and (.weak_output_routing.production_fix_candidates[] | select(.category == "chop_policy" and .software_next_step and .musician_payoff)) and (.weak_output_routing.fix_categories | index("chop_policy")) and (.weak_output_routing.fix_categories | index("bass_movement")) and (.source_family_coverage.missing_demo_ready_families | index("pad_noise")) and (.source_family_coverage.missing_demo_ready_families | index("bad_timing")) and (.demo_bank.unverified_candidate_ids | index("tonal-hook-rusharp-unverified-candidate")) and (.next_fix_categories | index("destructive_gesture")) and (.blockers[] | select(.code == "source_family_demo_ready_coverage_missing"))' "{{output}}/sound-quality-readiness-report.json"
