...
use riotbox_core::{
    ...
    session::{
        DAW_SESSION_AUDIBLE_OUTPUT_QA_GATE_ID, ExportArrangementPlacementRef, ExportArtifactRole,
...
fn daw_session_audible_output_proof_apply_smoke_mutates_only_receipt_gate() {
    ...
    assert_eq!(
        summary["receipt"]["daw_audible_output_gate"]["status"],
        "failed"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Value::Array(vec!["developer_proof_only".into()])
    );
