use riotbox_core::{
    ...
    session::{
        DAW_SESSION_HOST_IMPORT_QA_GATE_ID, ExportArrangementPlacementRef,
...
fn daw_session_host_import_proof_apply_attaches_gate_without_enabling_export() {
    ...
    assert_eq!(
        ...
        json!(["host_import_not_proven", "proof_blockers_present", "host_open_failed"])
    );
    assert_eq!(
    ...
    let passed_summary =
        daw_session_host_import_proof_apply_summary(&launch).expect("passed proof summary");
