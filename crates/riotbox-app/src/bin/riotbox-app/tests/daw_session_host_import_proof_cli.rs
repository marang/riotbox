use riotbox_core::{
    ...
    session::{
        ...
        ExportReceiptQaGateStatus, ExportReceiptState,
    },
};

#[test]
fn daw_session_host_import_proof_apply_attaches_gate_without_enabling_export() {
    let temp = tempfile::tempdir().expect("tempdir");
    let session_path = temp.path().join("session.json");
    let proof_path = temp.path().join("host_import_proof.json");
    let mut session = SessionFile::new(
        "daw-host-import-proof-apply",
        "riotbox-test",
    ...
    let missing_summary =
        daw_session_host_import_proof_apply_summary(&launch).expect("missing proof summary");

    assert_eq!(missing_summary["status"], "blocked");
    assert_eq!(missing_summary["mutates_session"], true);
    assert_eq!(
        missing_summary["readiness_blockers"],
        json!(["daw_writer_proof_missing", "missing_proof_file"])
    );
    assert_eq!(
    ...
    let failed_summary =
        daw_session_host_import_proof_apply_summary(&launch).expect("failed proof summary");
    ...
    assert_eq!(
        ...
        json!([
            ...
            "host_import_not_proven",
            "host_open_failed",
            "proof_blockers_present"
        ])
    );
    assert_eq!(
        failed_summary["receipt"]["daw_host_import_gate"]["status"],
        "failed"
    );
    ...
    let passed_summary =
        daw_session_host_import_proof_apply_summary(&launch).expect("out-of-order proof summary");

    assert_eq!(passed_summary["mode"], "daw_session_host_import_proof_apply");
    ...
    assert_eq!(
        ...
        json!(["daw_writer_proof_missing"])
    );
    assert_eq!(
        passed_summary["receipt"]["daw_host_import_gate"]["gate_id"],
        DAW_SESSION_HOST_IMPORT_QA_GATE_ID
    );
    assert_eq!(
        passed_summary["receipt"]["daw_host_import_gate"]["status"],
        "failed"
    );
    assert_eq!(
    ...
    saved_receipt.artifact_set.push(ExportArtifactSetEntry::daw_session_tempo_map(
    ...
    let ordered_summary =
        daw_session_host_import_proof_apply_summary(&launch).expect("ordered proof summary");
    ...
    assert_eq!(
        ordered_summary["receipt"]["daw_host_import_gate"]["status"],
        "passed"
    );
    assert_eq!(
...
#[test]
fn daw_session_host_import_proof_apply_blocks_without_daw_session_receipt() {
    let temp = tempfile::tempdir().expect("tempdir");
    let session_path = temp.path().join("session.json");
    let proof_path = temp.path().join("host_import_proof.json");
    save_session_json(
        &session_path,
        &SessionFile::new(
            "daw-host-import-proof-apply-no-receipt",
            "riotbox-test",
            "2026-06-03T19:45:00Z",
        ),
    )
    .expect("save session");
    fs::write(
    ...
    let summary =
        daw_session_host_import_proof_apply_summary(&launch).expect("no receipt summary");
