...
fn daw_session_writer_export_execute_ready_summary(
    ...
    json!({
        ...
        "action": latest_daw_session_writer_export_action_summary(state),
        "commit_records": latest_daw_session_writer_export_commit_records(state),
...
fn latest_daw_session_writer_export_action_summary(state: &JamAppState) -> Value {
    let action = state
        .session
        .action_log
        .actions
...
fn latest_daw_session_writer_export_commit_records(state: &JamAppState) -> Vec<Value> {
    let Some(action_id) = latest_daw_session_writer_export_action_id(state) else {
        return Vec::new();
    };
...
fn latest_daw_session_writer_export_action_id(
    state: &JamAppState,
