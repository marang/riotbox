...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let resample_outputs = if include_resample {
        ...
        println!(
            "base resample tap: mode={:?} routing={:?} availability={:?} source={:?} lineage={} generation={} variation={:?}",
        ...
        Some((damaged, base_tap, hard_tap, unavailable))
    } else {
        ...
        Some((damaged, Vec::new(), Vec::new(), Vec::new()))
    };
    ...
    if include_resample {
        let (_, base_tap, hard_tap, unavailable) =
        ...
        if variation_delta.rms <= 0.01 {
            return Err("post-resample hard variation collapsed to the base tap".into());
        }
