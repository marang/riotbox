...
use w30_live_path_render_modules::{
    ...
    pitch_dive_qualification::qualify_pitch_dive_v1, stereo_pad_exploration::explore_stereo_pad_v1,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let explore_stereo_pad = args.iter().any(|arg| arg == "--explore-stereo-pad-v1");
    ...
    if explore_stereo_pad {
        explore_stereo_pad_v1(&state, &normal_render, bpm, &output_dir)?;
        state.save()?;
        return Ok(());
    }
