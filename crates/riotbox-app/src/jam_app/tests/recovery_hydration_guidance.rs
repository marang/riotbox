...
#[test]
fn recovery_surface_reports_capture_now_artifact_hydration_blocker_guidance() {
    let dir = tempdir().expect("create temp dir");
    let target_path = dir.path().join("session.json");
    let blocked_autosave_path = dir
        .path()
        .join("session.autosave.capture-now-hydration-blocked.json");

    save_session_json(
        &target_path,
        &SessionFile::new("canonical", "riotbox-test", "2026-04-30T15:20:00Z"),
    )
    .expect("save canonical session");

    let action_id = ActionId(90);
    let capture_id = CaptureId::from("cap-02");
    let mut blocked_session = SessionFile::new(
        "capture-now-blocked",
        "riotbox-test",
        "2026-04-30T15:20:01Z",
    );
    blocked_session.snapshots.push(Snapshot {
        snapshot_id: SnapshotId::from("before-capture-now"),
        created_at: "2026-04-30T15:20:02Z".into(),
    ...
    blocked_session.action_log.actions.push(Action {
        ...
        undo_policy: UndoPolicy::Undoable,
        explanation: Some("capture source-window moment".into()),
    });
    blocked_session
        .action_log
        .commit_records
    ...
    save_session_json(&blocked_autosave_path, &blocked_session)
        .expect("save capture.now hydration-blocked autosave session");
    let autosave_before =
        fs::read(&blocked_autosave_path).expect("read capture.now autosave before scan");
    ...
    let blocked_candidate = surface
        ...
        .iter()
        .find(|candidate| matches!(candidate.trust, RecoveryCandidateTrust::RecoverableClue))
        .expect("capture.now hydration-blocked autosave candidate");
    ...
    assert_eq!(
        ...
        Some(
            RecoveryCandidateGuidance::SupportedArtifactReplayHydrationBlocked {
                detail: "capture.now action a-0090 cannot hydrate persisted artifact: missing storage path for capture cap-02".into(),
            }
        )
    );
    assert_eq!(surface.selected_candidate, None);
    assert_eq!(
        fs::read(&blocked_autosave_path).expect("read capture.now autosave after scan"),
        autosave_before
    );
}
