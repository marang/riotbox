...
fn committed_w30_filter_slam_persists_projects_and_replays_without_other_lane_changes() {
    ...
    assert_eq!(
        state.queue_w30_filter_slam(635),
        Some(QueueControlResult::Enqueued)
    );
    let committed = state.commit_ready_actions(
        CommitBoundaryState {
    ...
    let articulation = state
        ...
        .hook_articulation
        .as_ref()
        .expect("committed filter-slam articulation");
    assert_eq!(
        articulation.profile,
        riotbox_core::session::W30HookArticulationProfileState::FilterSlamV1
    );
    assert_eq!(articulation.capture_id, CaptureId::from("cap-01"));
    assert_eq!(articulation.started_at_beat, 80);
    let projected = state
        .runtime
        .w30_preview
        .pad_playback
        .as_ref()
        .and_then(|pad| pad.hook_articulation)
        .expect("projected filter-slam articulation");
    assert_eq!(
        projected.profile,
        riotbox_audio::w30::W30HookArticulationProfile::FilterSlamV1
    );
    assert_eq!(projected.started_at_beat, 80);
    assert_eq!(
        state
            .session
            .action_log
            ...
            .and_then(|action| action.result.as_ref())
            .map(|result| result.summary.as_str()),
        Some("filter-slammed cap-01 on W-30 pad bank-a/pad-01")
    );

    let serialized = serde_json::to_string(&state.session).expect("serialize filter-slam session");
    let restored: SessionFile =
        serde_json::from_str(&serialized).expect("restore filter-slam session");
    assert_eq!(
        restored.runtime_state.lane_state.w30.hook_articulation,
        state.session.runtime_state.lane_state.w30.hook_articulation
    );
    ...
    let plan = riotbox_core::replay::build_committed_replay_plan(&state.session.action_log)
        .expect("build filter-slam replay plan");
    let mut replayed = replay_base;
    replayed.action_log = state.session.action_log.clone();
    riotbox_core::replay::apply_replay_plan_to_session(&mut replayed, &plan)
        .expect("replay filter slam");
    assert_eq!(
        replayed.runtime_state.lane_state.w30.hook_articulation,
        state.session.runtime_state.lane_state.w30.hook_articulation
    );
