...
impl ExportReceiptQaGateResult {
    ...
    #[must_use]
    pub fn daw_session_writer_proof(
        passed: bool,
        blockers: &[String],
        artifact_roles: Vec<ExportArtifactRole>,
    ) -> Self {
        Self {
            gate_id: DAW_SESSION_WRITER_QA_GATE_ID.into(),
            status: if passed {
                ExportReceiptQaGateStatus::Passed
            } else {
                ExportReceiptQaGateStatus::Failed
            ...
            artifact_roles,
            summary: Some(daw_session_writer_summary(passed, blockers)),
        }
    }
...
fn daw_session_writer_summary(passed: bool, blockers: &[String]) -> String {
    if passed {
        return "DAW session writer proof accepted".into();
    }

    format!(
        "DAW session writer proof failed with {} blocker(s): {}",
        blockers.len(),
        blockers.join(", ")
    )
}
