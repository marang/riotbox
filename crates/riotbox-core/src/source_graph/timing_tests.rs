...
mod timing_tests {
    ...
    fn analysis_seed_from_case(case: &serde_json::Value) -> SourceTimingAnalysisSeed {
        source_timing_analysis_seed_from_fixture_case(case).expect("source timing analysis seed")
    }

    fn evaluation_target_from_case(case: &serde_json::Value) -> TimingFixtureEvaluationTarget {
        timing_fixture_evaluation_target_from_fixture_case(case)
            .expect("timing fixture evaluation target")
    }
}
