...
mod source_grid_output_drift_tests {
    ...
    fn pack_level_source_grid_output_drift_uses_generated_support_mix() {
        ...
        let mc202 = pulse_train(&grid, 0);
        let w30 = pulse_train(&grid, half_beat_frames);
        let generated_support_mix = pulse_train(&grid, half_beat_frames);

        let report = source_grid_alignment_report(&tr909, &mc202, &w30, &generated_support_mix, &grid);

        assert_eq!(report.tr909_source_grid_alignment.hit_ratio, 1.0);
        assert_eq!(report.mc202_source_grid_alignment.hit_ratio, 1.0);
        assert_eq!(report.w30_source_grid_alignment.hit_ratio, 0.0);
        assert_eq!(report.source_grid_output_drift.hit_ratio, 0.0);
    }

    #[test]
    fn pack_level_source_grid_alignment_exposes_mc202_drift_hidden_by_mix() {
        let grid = Grid::new(120.0, 4, 2).expect("grid");
        let half_beat_frames = frames_for_beats(grid.bpm, 1) / 2;
        let tr909 = pulse_train(&grid, 0);
        let mc202 = pulse_train(&grid, half_beat_frames);
        let w30 = pulse_train(&grid, 0);
        let generated_support_mix = pulse_train(&grid, 0);

        let report = source_grid_alignment_report(&tr909, &mc202, &w30, &generated_support_mix, &grid);

        assert_eq!(report.tr909_source_grid_alignment.hit_ratio, 1.0);
        assert_eq!(report.mc202_source_grid_alignment.hit_ratio, 0.0);
        assert_eq!(report.w30_source_grid_alignment.hit_ratio, 1.0);
        assert_eq!(report.source_grid_output_drift.hit_ratio, 1.0);
    }
