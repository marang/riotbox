...
#[test]
fn impact_pocket_leaves_the_slammed_tr909_lane_sample_exact() {
    let sample_rate = 48_000;
    let frame_count = 48_000;
    let plan = impact_pocket_test_plan(
        SourceMonitorRenderState::control_only(
            riotbox_core::action::SourceMonitorMode::Riotbox,
        ),
        0.0,
    );
    let mut drums_only = plan.clone();
    drums_only.mc202_render = Mc202RenderState::default();

    let runtime_mix = render_runtime_mix_offline(&drums_only, sample_rate, 2, frame_count);
    let direct_tr909 = render_tr909_offline(&drums_only.tr909_render, sample_rate, 2, frame_count);

    assert_eq!(runtime_mix, direct_tr909);
    let metrics = signal_metrics(&runtime_mix);
    assert!(metrics.active_samples > 0 && metrics.peak_abs > 0.001, "{metrics:?}");
}

#[test]
fn impact_pocket_leaves_source_only_output_sample_exact() {
    let sample_rate = 48_000;
    let frame_count = 48_000;
    let source = SourceAudioCache::from_interleaved_samples(
        "impact-pocket-source-only.wav",
        sample_rate,
        1,
        (0..sample_rate * 3)
            .map(|frame| (frame as f32 * 0.019).sin() * 0.28)
            .collect(),
    )
    .expect("source cache");
    let monitor = SourceMonitorRenderState {
        mode: riotbox_core::action::SourceMonitorMode::Source,
        source: Some(SourceMonitorAudioSource::from_cache(&source)),
        is_transport_running: true,
        tempo_bpm: 120.0,
    ...
    let candidate = impact_pocket_test_plan(monitor.clone(), 0.0);
    let mut control = impact_pocket_test_plan(monitor, 0.0);
    control.tr909_render.slam_enabled = false;
...
#[test]
fn impact_pocket_makes_only_local_room_and_preserves_runtime_mix_parity() {
    let sample_rate = 48_000;
    let frames_per_beat = 24_000;
    let frame_count = frames_per_beat * 4;
    let source = SourceAudioCache::from_interleaved_samples(
        "impact-pocket-blend.wav",
        sample_rate,
        1,
        vec![0.24; (sample_rate * 3) as usize],
    )
    .expect("source cache");
    let monitor = SourceMonitorRenderState {
        mode: riotbox_core::action::SourceMonitorMode::Blend,
    ...
    let mut plan = impact_pocket_test_plan(monitor, 0.0);
    plan.mc202_render = Mc202RenderState::default();
    let mut drums_only = plan.clone();
    drums_only.source_monitor_render.source = None;
    let mut bed_only = plan.clone();
    bed_only.tr909_render = Tr909RenderState::default();

    let full = render_runtime_mix_offline(&plan, sample_rate, 2, frame_count);
    let realtime = render_runtime_mix_realtime_simulation_offline(
        &plan,
        sample_rate,
        2,
    ...
    let drums = render_runtime_mix_offline(&drums_only, sample_rate, 2, frame_count);
    let bed = render_runtime_mix_offline(&bed_only, sample_rate, 2, frame_count);
    let focused_bed = full
        .iter()
        .zip(&drums)
        .map(|(mixed, drum)| mixed - drum)
        .collect::<Vec<_>>();

    let owner_hold_end = (frames_per_beat as f64 * 0.03125) as usize * 2;
    let owner_focused = signal_metrics(&focused_bed[..owner_hold_end]);
    let owner_control = signal_metrics(&bed[..owner_hold_end]);
    let outside_start = frames_per_beat / 2 * 2;
    let outside_end = frames_per_beat * 3 / 4 * 2;
    let outside_focused = signal_metrics(&focused_bed[outside_start..outside_end]);
    let outside_control = signal_metrics(&bed[outside_start..outside_end]);

    assert_eq!(full, realtime);
    assert!(owner_control.rms > 0.001, "control bed was silent: {owner_control:?}");
    assert!(
        owner_focused.rms < owner_control.rms * 0.32,
        "named-owner collision did not reach the frozen pocket: focused={owner_focused:?} control={owner_control:?}"
    );
    assert!(
        (outside_focused.rms - outside_control.rms).abs() < 0.000_001,
        "impact pocket leaked outside its local region: focused={outside_focused:?} control={outside_control:?}"
    );
    assert_eq!(signal_metrics(&full).clip_count, 0);
}

fn impact_pocket_test_plan(
