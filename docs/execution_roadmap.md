...
- `pressure_lift` and related rise / restore gestures are source-aware
  ...
  - RIOTBOX-1318 strengthens the same sparse-bass-pressure path for routed
    `bass_movement` fixes: source-derived bass contour expansion now rejects
...
- RIOTBOX-1318 strengthens routed `bass_movement` cases: sparse-bass-pressure
  ...
  pressure section, and raise sparse low-band/bass-dominance gates in the
...
- RIOTBOX-1366 extends that reconciliation to `destructive_gesture`: stale
  ...
  controls.
- RIOTBOX-1367 extends that reconciliation to `mix_bus`: stale source-masked or
  support-buried weak fixtures remain regression controls, but readiness must
  mark mix bus as stale fixture-only when current professional-suite generated
...
- large professional-output JSON contracts move into named repo-local validators
  ...
  - RIOTBOX-1391 applies the same extraction to general listening-review label
...
The v1 artifact remains negative evidence. The exact v2 rerender then proved
that the generated journey and restart recall are sample-exact to W-30-only
output while TR-909, MC-202, and Source Monitor remain silent. Its first review
identified and removed one artificial review-only separator; the corrected
29-second artifact received `keep`, retained a clear recognizable transformed
hook, and preserved the already-qualified Pitch Dive unchanged. This closes the
...
W-30 hook and existing TR-909 support sound good and well matched as a composite
loop. This is not isolated W-30 or a separate hardness/seam/source-recognition
