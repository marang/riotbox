...
- never read `docs/research_decision_log.md` wholesale during normal
  ...
  exact `rg` terms, targeted line ranges, or bounded MemPalace search
- use `just decision-search "query"` for normal research-decision retrieval;
  it syncs the MemPalace corpus, uses MemPalace only when the index is already
  fresh, and falls back to bounded `rg` output instead of auto-mining in the
  middle of implementation work
...
- MemPalace:
  ...
  - use bounded MemPalace search for large decision/history files such as
    `docs/research_decision_log.md` when semantic retrieval helps, but keep
    timeouts short and fall back to targeted `rg` if MemPalace starts mining or
    blocks
  - prefer `just decision-search "query"` for the research decision log; use
    `just decision-search-mine "query"` only when semantic retrieval is worth a
    bounded re-index attempt
