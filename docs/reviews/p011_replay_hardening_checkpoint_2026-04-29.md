...
- Stage-style Jam probe adds a longer generated W-30 source-diff output plus a generated app-level multi-boundary observer/audio correlation gate: `scripts/validate_stage_style_jam_probe.sh`.
...
- **Smoke-level audio QA is broader**: first-playable, stage-style, observer/audio correlation, and offline render reproducibility gates now run under `just audio-qa-ci`. The stage-style gate now asserts generated summary `commit_count` plus `Phrase` / `Bar` / `Beat` boundary coverage, but remains a bounded probe.
