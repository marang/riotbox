...
- **Replay planning is inspectable and partially executable**: dry-run and convergence summaries can show origin, anchor, suffix, unsupported command diagnostics, and full-replay requirements without becoming a second replay engine. App runtime warnings now surface unsupported replay commands at the restore/debug seam, and core replay can execute a target suffix against already-hydrated anchor state with all-or-nothing semantics.
