...
- Stage-style Jam probe adds a longer generated W-30 source-diff output plus a dedicated multi-boundary observer/audio correlation gate: `scripts/validate_stage_style_jam_probe.sh` with fixture `crates/riotbox-app/tests/fixtures/stage_style_jam_probe/events.ndjson`.
...
- **Replay planning is inspectable and partially executable**: dry-run and convergence summaries can show origin, anchor, suffix, unsupported command diagnostics, and full-replay requirements without becoming a second replay engine. App runtime warnings now surface unsupported replay commands at the restore/debug seam, and core replay can execute a target suffix against already-hydrated anchor state with all-or-nothing semantics. App QA now proves that an explicit restore-runner scaffold can feed downstream TR-909 render and source-backed W-30 preview projection from test-owned hydrated anchors, app recovery code can request read-only target dry-run summaries, runtime-view data can expose restore-readiness status/anchor/suffix/unsupported labels without mutating state, and unsupported suffixes are covered by an all-or-nothing rejection plus visible restore warning fixture.
- **Smoke-level audio QA is broader**: first-playable, stage-style, observer/audio correlation, and offline render reproducibility gates now run under `just audio-qa-ci`. The stage-style gate now asserts generated summary `commit_count` plus `NextBar` / `NextBeat` / `NextPhrase` boundary coverage, but remains a bounded probe.
- **Crash failure behavior is explicit at MVP level**: partial JSON is rejected instead of silently repaired or misloaded.

## Remaining P011 Gaps

- **Full replay runner is not implemented**: Riotbox still hydrates from persisted runtime state plus validation rather than replaying every action from origin into a reconstructed state. Target suffix execution exists as a scaffolding boundary, but real snapshot payload hydration is still missing.
- **Snapshot convergence is partially execution-proven**: current helpers prove selected anchor and suffix scope. The safe structural replay subset, MC-202 phrase-family subset, first TR-909 support-move subset, first W-30 cue subset, and first Scene launch/restore subset now compare origin replay against materialized anchor-plus-suffix replay. Graph-aware Scene movement hydration now exists for the launch/restore subset. Artifact-producing W-30 actions are explicitly rejected without partial mutation until capture/resample hydration exists. Real snapshot payload hydration and broader W-30 / scene replay are still open.
...
- **Long-run/stage-run hardening is still bounded**: `just stage-style-jam-probe` is a longer CI probe with dedicated multi-boundary control-path evidence, not a soak test or extended live-session simulation.

## Recommended Next Slices

1. Continue expanding replay-from-origin vs latest-snapshot convergence one replay-safe musical action family at a time, with output-path proof where audible state is affected. MC-202 phrase replay, the first TR-909 support-move family, the first W-30 cue subset, and graph-aware Scene launch/restore movement replay are now covered; W-30 artifact hydration and broader Scene behavior remain open.
...
4. Expand stage-style QA only where it catches new risk: longer action sequences, stronger summary assertions, or real user-session observer correlation.
