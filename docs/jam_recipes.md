...
- `metrics.mc202_source_grid_alignment.hit_ratio` is the lane-specific MC-202
  proof; values below `0.5` reject source-derived bass-stem publication. A
  passing origin additionally requires the applied source-expression plan,
  source-contour delta, bass-pressure proof, and `source_failure_fallback:
  false`.
...
- Riotbox can generate a bounded TR-909, MC-202, and W-30 Feral grid pack with
  manifest-backed output metrics. MC-202 now renders a source-derived phrase
  plan and publishes that origin only through the complete fail-closed evidence
  set; this still is not a musician stem-export action or Session receipt.
