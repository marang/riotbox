...
Date: 2026-07-23
Topic: post-resample W-30 damage must mutate the active source-backed tap instead of replacing its focus
Phase: P023 / Controlled Expansion
Question: how should Riotbox recover recognizable level and add a hard live variation after RIOTBOX-1408 without introducing another action system, scripted arrangement, or synthetic voice?
Decision: retain `w30.apply_damage_profile` as the performer gesture owner. Project the most recently materialized lineage-ready resample across later pad-focus changes only while current focus is that resample or one of its lineage captures; unrelated later material deactivates it. Classify a damage commit as typed `hard_damage` only when it occurs after that resample and targets the resample or one of its lineage captures. Carry the variation, committed action-log revision, and intensity through the existing coherent callback state. Project the complete committed resample artifact into a bounded `16384`-sample full-duration mono proxy. Base owns uninterrupted phrase flow so it reaches later source material; `hard_damage` owns the chopped role with an immediate rate dive, cursor changes across the whole phrase, deliberate rhythmic holes, denser source-derived retriggers, and stronger saturation. Missing material remains unavailable and silent.
Why: the first source-backed tap was real but a direct A/B sounded like a timid generic tap and lost Beat03 recognition. The first RIOTBOX-1422 attempt doubled that grain to `8192` source frames, lifted it, and added a typed hard path, yet structured listening still rejected all intended outcomes. The `1.389`-second input had been reduced to about `186 ms`; base repeatedly restarted it and hard only changed positions inside it. Treating raw-signal delta as sufficient therefore produced a musical false positive. A new action would still duplicate already-complete queue/commit/Session/replay ownership, while a fixed multi-bar effect sequence would freeze QA choreography into product behavior.
Evidence: the rejected structured verdict records `source_lost`, missing hook, no strongest element, and no playable base/hard role in `/tmp/riotbox-1422-listening-review/verdict/review.json`. The revised exact RuntimeMix candidates measure base/hard `-18.1/-19.6 LUFS` for Beat03, `-14.1/-16.1` for Beat08, and `-15.1/-18.0` for Beat20; all are unclipped and source-distinct, with missing-source controls silent. Immediate resample-input-to-base 20 ms RMS-envelope correlations are `0.883351`, `0.949029`, and `0.942699`; base-to-hard correlations are `0.061049`, `0.065543`, and `0.048294`. Focused regressions prove base never grid-resets its phrase while hard owns six attacks and two deliberate holes per eight-step cycle. App regression proves damage may refocus the source pad while active resample identity remains `cap-02`; snapshot replay retains the typed variation.
Consequences: capture history and the committed action log remain the only product truth; no app-local active-resample model is introduced. Jam/Capture/Log and observer surfaces expose the typed variation and activation evidence. Human listening must compare the original-source context separately from the actual committed resample input; automated envelope/source evidence cannot promote the revised candidate without a fresh human keep verdict.
Status: proposed pending structured listening
...
Evidence: after three recorded rotations, the tracked contract records nineteen OpenGameArt source entries. The active corpus still contains fifteen core-family-eligible sources across fourteen independent source packs, plus one confirmed `dense_full_mix` stress source; three additional eligible entries remain only as retired lineage. The active development matrix spans all six core families plus the stress family, and both holdout sets remain multi-family. The local qualification inventory binds author, page/download provenance, CC0 license, excerpt offset, and SHA-256. `just source-holdout-rotation-fixtures` rejects candidate family collapse, insufficient or reused holdouts, retired lineage used to satisfy active-corpus minimums, consumed holdouts left marked unseen, unsafe paths, commercial-reference leakage, narrow source-pack collapse, and missing files. `just source-holdout-rotation-local-files` verifies every local derivative by SHA-256, 48 kHz stereo PCM16 format, duration, and clipping boundary.
Consequences: RIOTBOX-1423 is a `contract_enabler`, not musical quality proof. RIOTBOX-1422 must use the development matrix for implementation choices and reserve fresh holdouts for cross-family acceptance. Any holdout-driven follow-up invalidates that source's unseen status until rotation is recorded and replacement evidence exists. A holdout may be rendered only after the candidate and gates are frozen; repeated technical rendering across changing candidate generations conservatively consumes practical freshness when causal influence cannot be disproved. If consumption happens before raw-source suitability is settled, the retired entry remains `unverified`; rotation must not manufacture a source-quality verdict, and a technical reviewer may own that governance record without implying a human pass. The ignored CC0 audio remains local and unredistributed.
Status: accepted
...
Date: 2026-07-25
Topic: W-30 Base resample cycles must restart on their quantized transport boundary
Phase: P023 / Controlled Expansion
Question: how should the source-backed W-30 Base path respond when a nominal one-bar artifact repeats a source kick just after the grid and is heard as an echo that pulls the beat out of time?
Decision: fail source-window materialization closed when the complete requested quantized duration does not fit before source EOF; never label a truncated remainder as the requested capture. With valid transport tempo, infer the hydrated artifact's bounded whole-beat duration and render its continuous Base cycle over exactly that many transport beats. Remove generation/grit playback-rate offsets from the continuous resample cursor and accumulate its phase in `f64`. Keep performer-owned Hard retriggers on their typed source-local onset plan.
Why: the rejected review renderer asked for a one-bar capture with only three beats remaining, and the capture helper silently clamped it to `1.389365 s`. Generation/grit rate modulation then wrapped that artifact after about `3.05` beats, placing a second source kick about `47.85 ms` after the grid attack. Listening correctly identified the result as a doubled echo and a timing failure, not merely a weak sound.
Evidence: the corrected renderer captures a complete `1.842109 s` four-beat Beat03 window. A capture regression rejects the truncated one-bar request. A callback regression proves the complete Base artifact restarts within one output frame of the transport bar, including after changing the cursor to double precision. The corrected waveform has one consistent kick body whose strongest rise lands about `5–15 ms` after each bar boundary and no separate late restart. Seven development/holdout exact-path cases remain non-silent and unclipped; missing-source controls remain digital silence; maximum absolute pairwise Hard-envelope correlation is `0.550674`, below the existing `0.95` collapse ceiling.
Consequences: Base remains a continuous source-backed phrase anchor rather than a grid-retrigger voice, but its cycle length is grid-safe. Pitched or rate-dived behavior must be an explicit typed performer variation with its own timing contract; generation depth or grit may not make Base free-run against transport. The corrected artifact remains `human_verdict: unverified` until structured listening confirms that the doubled kick is gone and then judges Hard usefulness.
Status: accepted pending structured human listening
...
Date: 2026-07-25
Topic: typed W-30 Hard policies must create role-specific articulation rather than only waveform difference
Phase: P023 / Controlled Expansion
Question: how should Riotbox respond when grid-safe Base passes listening but the source-backed Hard half is audibly not hard?
Decision: preserve the corrected Base byte-for-byte. For `source_transient_chop`, retrigger a tempo-derived envelope at each selected local source onset and decay it to `0.03` within `0.55` of one eighth-note step so inactive source-derived mask slots become real arrangement space. Strengthen source-reactive edge/drive for both Hard policies and apply a bounded `1.12` whole-path gain compensation only to the gated transient policy. For continuous `source_texture_bite`, retain uninterrupted playback and add bounded wavefold/quantization of the real source waveform instead of inventing a trigger grid.
Why: the reviewed Hard candidate was technically different from Base and about `1.2 LU` louder, but listening correctly found it was not genuinely Hard. Its envelope decay was fixed at `1.0`, leaving only `0.7%` silence versus `1.0%` in Base; the trigger mask jumped between source cursors without creating chopped attacks, holes, or performance impact. A level lift or another off-grid layer would repeat earlier failures.
Evidence: Beat03 Base remains byte-identical at `13d5943f0d2c222f80c279991dfbf3593b6e155b99795c916661bb3565f64a4c`. Beat03 Hard measures `45.75%` silence, crest factor `7.789`, peak `0.709396`, and zero clips. Beat03 plus seven development/holdout cases are non-silent and unclipped, all missing-source controls are digital silence, and maximum absolute pairwise Hard-envelope correlation is `0.183690`. Seven transient policies produce relative Base-to-Hard RMS deltas from `1.062192` to `1.248608`; the continuous texture policy produces `0.152667` without grid triggers. Structured listening of exact A/B SHA-256 `1ca4b7bdf5029ff60bfc8cc2cb2bb7b24639e04c2010c6e2ff56cd14b5c32ee4` rejected the candidate: it was choppier and emptier, not harder.
Consequences: waveform delta, louder peaks, and more silence are not accepted as evidence of Hard articulation. The gate remains a deterministic articulation transform but is not a listening-approved Hard recipe. Transient Hard still needs source-local attack, retained role-appropriate body, controlled bite, stable pulse, and musician-useful contrast. Texture Hard still needs source-derived continuous destructive timbre. Both remain silent when source material is unavailable. RIOTBOX-1422 must change the causal mechanism before another candidate instead of nudging the rejected gate/gain/drive constants.
Status: rejected by structured human listening
...
Date: 2026-07-26
Topic: perceptual Hard requires source-adaptive attack, retained body, and controlled nonlinear bite rather than more gating
Phase: P023 / Controlled Expansion
Question: what research-backed mechanism and QA contract should replace another round of W-30 Hard magic-number tuning after the gated candidate sounded choppier and emptier but not harder?
Decision: preserve the grid-safe Base byte-for-byte and treat `hard`, punch, roughness, bass/low-body pressure, midrange/hook bite, and arrangement impact as separate perceptual domains. For the next transient Hard experiment, derive a bounded source-local attack/body plan outside the realtime callback, retain a time-aligned dry/body path, and test controlled source-relative band-limited nonlinear processing on the attack path. Keep gating/chokes as arrangement articulation rather than hardness proof. Test source-derived roughness only as a later bounded optional mechanism. Do not require bass unless a typed owner assigns it. Use `docs/engineering/perceptual_hardness_and_musical_impact.md` as the research and experiment contract.
Why: published perceptual models describe hardness and punch through combinations of attack duration/strength, attack spectrum and bandwidth, midband level, percussive-to-harmonic relation, frequency band, signal duration, and microdynamics. Roughness in fast modulation bands can increase salience or aversion but is not equivalent to punch or quality. Production references likewise separate transient contour, sustain/body, band-selective distortion, low-end support, parallel blend, and output compensation. The exact Riotbox audit found the rejected Hard candidate raised only the first `0–10 ms` onset RMS, then removed `48.3%` at `10–40 ms`, `69.0%` at `40–120 ms`, and `91.0%` at `120–200 ms`; attack/body ratio rose from `6.8` to `39.0`. The signal became a short bright click without physical or midrange body.
Evidence: exact A/B artifact SHA-256 is `1ca4b7bdf5029ff60bfc8cc2cb2bb7b24639e04c2010c6e2ff56cd14b5c32ee4`; structured verdict SHA-256 is `4a9cf47ce50d526b47e5bd5eb4aa468519bc02a526c9220d973f345ed7d5f633` with `human_verdict: reject`. The Base sibling remains `13d5943f0d2c222f80c279991dfbf3593b6e155b99795c916661bb3565f64a4c`; Hard is `a0d58690e5fe355e66a9f032557441034b28d823a202326d97760b0950dcff1e`. The research synthesizes primary timbral-hardness, punch, perceptual-attack, roughness, bass-drum-hardness, consonance, and groove studies plus official production documentation. It defines native and loudness-matched comparison, onset-local temporal/spectral/body metrics, multi-family development and holdout proof, and structured listening as separate gates.
Consequences: no single hardness score or magic number can promote a candidate. Source analysis, onset localization, decomposition, band choice, and recipe selection stay outside the callback; the callback receives bounded typed state and remains free of I/O, allocation, model calls, and unbounded analysis. `w30.apply_damage_profile` remains the gesture owner. The code audit also requires source-local edge-history priming, a bounded source/artifact revision for active PCM replacement, safe fresh-stopped and seek/restore state, stronger onset/body and boundary-jump QA, and a callback snapshot benchmark before expanding DSP cost. Another human playback waits until one causal mechanism passes at least five development sources across four typed families plus valid untouched holdouts and the exact artifact is technically analyzed and assigned. `oga_illin_robotic` is consumed development material: after five successive Hard iterations, its `15.3%` texture delta was used to raise the shared regression threshold from `0.005` to `0.02`. The rotation manifest now retires it and installs the independent, unheard CC0 `oga_riintron_fat_groove_drums` replacement. Two other reserve cases repeatedly rendered across changing candidates are conservatively retired and replaced by the unheard `oga_hornpipe2_prehistoric_drums` and `oga_centurion_chippy_melody` cases. All replacements remain provisional and acquisition is not quality proof. Reusable guidance for harmonic coherence, punch, loop interest, timing, and performance is now durable project documentation rather than chat-only context.
Status: accepted research direction; implementation and human verdict pending
...
Date: 2026-07-27
Topic: H13 adds one source-selected reverse-into-impact gesture without replacing H12 body ownership
Phase: P023 / Controlled Expansion
Question: how can the W-30 Hard path add stage-useful anticipation and retained impact body without another global gain, brightness, or fixed-pattern iteration?
Decision: freeze `source_reverse_into_impact_v1`. Rank existing source-selected transient slots from local 0–20 ms head and 20–100 ms body evidence, reverse 120 ms of the strongest impact into its preceding eighth-note boundary, return forward on-grid, and articulate only that impact's actual post-H12 20–100 ms callback output. Derive body gain toward 90% of the same hit's head RMS within `1.12..=1.40`; derive impact and pickup compensation separately from source energy. Do not lengthen H12's attack slice, assign bass ownership, alter texture policy, or emit a missing-source fallback.
Why: extending a selected attack replay to 100 ms reduced the already valid H12 body on short or textural hits, so a larger gain merely compensated for a self-created loss. Applying bounded articulation to the rendered H12 impact preserves source ownership and works across sources with different attack lengths. A source-relative 90% body/head target gives stress material enough retained body while already body-rich material remains at the minimum bound.
Evidence: the frozen nine-source development matrix spans dense break, sparse drums, dense-full-mix stress, tonal riff, pad/noise, and weak-source behavior. Six eligible transient cases choose different source slots and pass the H12-to-H13 body gate from `1.0532x` to `1.2900x`, pickup delta from `0.6994` to `7.6538`, and inherited level limits. Weak and texture cases remain explicitly unavailable for H13; missing-source output remains digital silence. Exact values, hashes, gates, and untouched holdout-b membership are recorded in `docs/benchmarks/w30_resample_h13_characterization_v1.json`.
Consequences: H13 claims drum/transient or midrange/hook body plus arrangement/performance anticipation, not bass pressure or complete musical quality. Any candidate or gate change invalidates holdout-b as H13 evidence. Human listening remains mandatory and may still reject the technically valid gesture as weak, incoherent, or not loop-worthy.
Status: rejected by frozen holdout before human listening; inherited H12 level calibration exceeded the gate
...
Date: 2026-07-27
Topic: exact W-30 hit calibration must preserve the owned hit and lower only between-hit material
Phase: P023 / Controlled Expansion
Question: how should the post-H13 generation correct source-dependent H12 level overshoot without weakening the selected attack/body or tuning another fixed output scalar against one source?
Decision: replace the fixed whole-path `0.94` hit-shaper gain with a bounded source- and tempo-specific calibration performed outside the realtime callback. Render four complete eight-step cycles through the exact callback at 48 kHz stereo, target `1.20x` whole-path RMS under the unchanged `1.30x` ceiling, and cache the result by all source/recipe inputs. Three exact endpoint renders feed a source-specific energy model; a bounded in-memory search proposes the gain and at most four exact callback refinements correct model error or retain the `1.10x` head and `1.15x` body role floors. Every time-window measurement covers all four rendered cycles. In the callback, ramp over 2.5 ms into 20 ms of real full-unity pre-hit context, preserve the selected 0–100 ms hit at unity, retain the H12 `0.94` level through 100–200 ms, fade to the calibrated between-hit gain over 10 ms, and expose both attempted/evaluated and successful calibration state in runtime/observer evidence. Cache matching evaluated rejections as well as successful calibrations. Raise the H14 selected-body target from the frozen H13 90%/`1.40` cap to 95%/`1.45` only for the already source-selected 20–100 ms impact body.
Why: frozen H13 holdout forensics showed `oga_iamoneabe_tryme` already reached `1.41339x` at H12 and H13 added only `1.02599x`; the reverse gesture was not the primary failure. A global reduction corrected level but collapsed 40–200 ms body. Preserving the hit while lowering its surroundings creates physical contrast at the responsible layer and avoids another global-loudness workaround.
Evidence: the consumed Tryme development case now uses output gain `0.639841` and passes Base-to-H12 level `1.25878x`, filtered head `1.16759x`, filtered body `1.30463x`, H13 impact body `1.05539x`, reverse-pickup delta `1.97044`, and complete Base-to-H13 level about `1.29208x`. The final ten-case development matrix covers dense break, dense-full-mix stress, sparse drums, tonal riff, pad/noise, weak-source rejection, and continuous texture policy. All cases pass unchanged product-path gates; weak/unowned cases do not run the exact hit calibration. Unit regressions prove exact callback activation, matching-input positive and negative cache reuse, distinct primary-hit/following-body/between-hit gains, click-safe pre-roll, and calibration measurement across every rendered cycle.
Consequences: exact calibration is control-plane work and may run only when source/tempo/recipe inputs change; normal runtime refreshes reuse cached evidence. The callback gains only bounded counters and arithmetic. `0.94` remains versioned H12 schema history, not a universal output level. H14 remains `human_verdict: unverified` until a frozen fresh holdout passes technical gates and structured listening judges musical hardness, coherence, character, and loop usefulness.
Status: accepted implementation direction; fresh holdout and human verdict pending
...
Date: 2026-07-27
Topic: a frozen audio holdout must exercise the candidate's causal mechanism
Phase: P023 / Controlled Expansion
Question: can H14 pass fresh holdout when its only renderable source passes inherited W-30 gates but does not select the new exact hit-window calibration?
Decision: reject H14 before human listening. Run source timing before candidate rendering and do not manufacture `--bpm` confirmations for manual-confirm-only or unavailable sources. Accept the provider-declared 140 BPM dense-break source for one frozen render, but classify its result as inherited-path evidence only because it selected `source_transient_chop` with `low_impact_recipe=unavailable`, `exact_callback_evaluated=false`, and `exact_callback_calibrated=false`. Consume Holdout A and prohibit H14 retuning against it.
Why: a technically clean output that bypasses the changed mechanism cannot prove that mechanism generalizes. Counting non-selection as success would turn rotating holdout evidence into an escape hatch and repeat the project's earlier pattern of logs claiming progress without proving the audible product path under review.
Evidence: the sole eligible render passes Base-to-Hard level `1.0498x`, H12-to-H13 impact body `1.066922x`, reverse-pickup relative delta `1.073662`, unclipped Hard output, and digital-silent missing-source control. Three other sources report manual-confirm-only grids, while the bad-timing one-shot reports timing unavailable. Exact hashes and all pre-candidate outcomes are frozen in `docs/benchmarks/w30_resample_h14_characterization_v1.json`.
Consequences: H14 remains useful development code but is not listening-eligible or accepted product progress. The two-set rotation is restored with five new local ignored CC0 reserve derivatives that were selected from provider metadata and checked only for archive membership, format, duration, peak, clipping, and hash. H15 freezes the byte-identical H14 implementation and gates as a coverage retry against untouched Holdout B; it adds no DSP or selection constants. Its evaluation must contain causal coverage of the selected `source_hit_shaper_v3` path. Replenishment metadata may establish legal and technical eligibility, but must not pre-listen or pre-render reserve audio.
Status: rejected by frozen holdout coverage before human listening
...
Date: 2026-07-27
Topic: trusted BPM does not imply trusted downbeat when the analyzer selects a conflicting tempo alias
Phase: P023 / Controlled Expansion
Question: should H15 force a provider-declared 190 BPM source through the exact product path when the Rust probe selects 141.51 BPM with high drift?
Decision: no. Preserve the existing contract: BPM-only input confirms only a matching Rust primary hypothesis within 1 BPM. The paired musician-manual exception requires both independently known BPM and downbeat phase. Reject H15 before candidate rendering because the holdout provides no trusted downbeat declaration; do not invent phase zero, substitute the conflicting analyzer tempo, or loosen matching tolerance against the consumed source.
Why: 141.51 BPM is close to the 3:4 alias of 190 BPM, so the mismatch is musically plausible rather than random. It still does not establish bar-one phase. Letting provider tempo silently override the typed Source Graph would recreate a second timing truth and could produce an exact callback render that is confidently off-grid.
Evidence: the exact product path fails before persistence with `explicit source BPM 190.00 does not match Rust timing candidate 141.51 within 1.00 BPM`. The other three musical Holdout B sources remain manual-confirm-only with high drift, and the impact one-shot is timing unavailable. No H15 candidate WAV was produced, `source_hit_shaper_v3` was not evaluated, and no human listening was requested.
Consequences: consume Holdout B and restore the two-set rotation before another frozen coverage generation. Future acquisition for this gate should prefer sources whose provider metadata includes tempo and whose source start is independently documented as the downbeat, or whose analyzer candidate matches the provider tempo. This is an acceptance-reachability finding, not permission to weaken source timing or claim H14 musical quality.
Status: rejected by frozen holdout timing before candidate rendering
...
Date: 2026-07-27
Topic: stop byte-identical holdout retries when trusted-grid reachability prevents causal audio coverage
Phase: P023 / Controlled Expansion
Question: should Riotbox keep consuming fresh holdouts after H14 bypassed the changed recipe, H15 failed tempo confirmation, and a metadata-targeted H16 source still could not reach candidate rendering?
Decision: no. Reject H16 before human listening and stop byte-identical coverage retries. Preserve H14 implementation and gates as development evidence, but require a separate acceptance-reachability enabler before another frozen audio generation. That enabler must account for both trusted source timing and causal `source_hit_shaper_v3` selection without weakening BPM/downbeat truth, pre-rendering future holdouts, or treating recipe non-selection as a pass.
Why: the H16 source was selected before freeze from provider 140 BPM, hard-hitting, and start-stop metadata specifically to improve reachability. The Rust probe nevertheless selected 154.64 BPM with weak downbeat, high drift, and manual-confirm-only use. No independent downbeat phase existed. Repeating the same process with more sources would spend acceptance evidence while leaving the underlying reachability contract unchanged.
Evidence: H14 produced one inherited-path render with `exact_callback_evaluated=false`; H15 failed before persistence on provider 190 versus Rust 141.51 BPM; H16 failed before candidate rendering on provider 140 versus Rust 154.64 BPM. Every other H15/H16 musical source required manual confirmation with high drift, while both one-shot controls were unavailable. No H15 or H16 candidate WAV exists and no structured listening was requested.
Consequences: restore the two-set source rotation once, then freeze it. Create a bounded contract enabler whose directly enabled outcome is a fresh RIOTBOX-1422 product-path holdout that can prove or reject H14 exact calibration causally. The enabler may add diagnostic prequalification over development material or explicit holdout eligibility metadata, but cannot inspect future holdout audio, add a shadow timing system, or weaken Session-confirmed grid ownership.
Status: accepted blocker diagnosis; enabler required before further candidate generation
...
Date: 2026-07-27
Topic: W-30 holdout reachability is a typed non-listening gate before candidate WAV generation
Phase: P023 / Controlled Expansion
Question: how can a frozen W-30 Hard holdout prove the exact hit-shaper mechanism without repeatedly consuming sources that cannot reach its timing or recipe path?
Decision: add `riotbox.w30_reachability_preflight.v1` to the existing exact W-30 renderer. Before any candidate review WAV is written, classify the shared Rust timing result and explicit BPM/downbeat route, then run the normal Source Graph, Session, queue, commit, capture-promotion, internal-resample, and Hard projection path. Candidate generation is eligible only when the product-selected state is `source_transient_chop` plus `source_hit_shaper_v3`, exact callback calibration is applicable, evaluated, and successful. Policy non-selection, unavailable timing, and BPM mismatch without an independently confirmed downbeat are explicit ineligible outcomes. A paired musician/provider BPM and downbeat may install the existing typed manual-grid hypothesis even when the analyzer selected an alias. Preflight-only runs emit typed JSON and no candidate review WAV. The holdout contract requires two prequalified sources across two families and at least one exact applicable case; screening cannot listen, tune DSP/selection/gates, or assume downbeat phase zero.
Why: H14 reached only an inherited recipe, while H15 and H16 failed timing before persistence. A causal acceptance set therefore needs stratified reachability evidence without turning diagnostics into a second selector or weakening the 1 BPM confirmation contract. The shared applicability predicate now lives on `W30ResampleTapState` and is used by both projection calibration and preflight.
...
Date: 2026-07-27
Topic: derive downbeat phase from source evidence when external tempo conflicts with the analyzer
Phase: P023 / Controlled Expansion
Question: may externally supplied BPM become trusted product timing without a musician-supplied downbeat when the Rust primary is missing or selects a conflicting tempo?
Decision: yes, but only through a typed `TempoGuided` Source Graph hypothesis. Keep the external BPM fixed, derive every phase candidate from a real source onset modulo the bar period, require at least eight onsets, aligned onset and strength support, at least 30% complete-bar downbeat coverage, a 0.48 composite score, a 0.020 winning margin, and drift no worse than 45 ms mean / 90 ms maximum. Emit only complete bars, preserve analyzer hypotheses and warnings, and bind hypothesis identity to BPM, selected phase, and actual onset anchor. Do not emit a user-confirmation action for machine evidence.
Why: provider/project tempo can be independently trustworthy while the rendered audio produces a metrical alias, but BPM alone cannot justify phase zero. Treating source-derived phase as `Manual` would invent musician trust; treating it as the analyzer primary would hide that tempo came from outside Riotbox. The separate type preserves both facts and fails closed on flat, sparse, drifting, or phase-ambiguous material.
Evidence: the final development matrix spans six local CC0 cases across dense-break, tonal-riff, sparse-drums, pad/noise, and bad-timing families. 8-Bit Victory Loop reaches the exact Source Graph product route at project-derived 180 BPM with a 0.010667-second source phase, 4/11 complete-bar hits, 3.11 ms mean drift, and a 0.10493 score margin. Drama remains rejected at provider 130 BPM with only 2/7 bar hits and no winning margin. A sparse drum challenge rejects for excessive drift, pad/noise and one-shot controls reject for insufficient evidence, and matching-primary development cases retain their prior confirmation route. Synthetic tests prove non-zero phase identity, flat-accent ambiguity, sparse-tonal support, insufficient-onset rejection, complete-bar output, and preservation across Source Graph save/restore and observer projection.
Consequences: `--source-bpm` now selects either the existing matching-primary user-confirmation route or the source-backed `TempoGuided` route; explicit BPM plus downbeat remains the only `Manual` route. W-30 preflight serializes the typed evidence and verifies product BPM and phase before Hard projection. W-30 DSP plus the 0.18 / 1.40 / 0.30 hit-shaper gates remain unchanged. RIOTBOX-1422 may use this enabler for one fresh H24 exact-path attempt after branch review and full QA; untouched reserve material remains inaccessible until then.
Status: accepted bounded timing enabler; H24 fresh exact-path proof pending
...
Date: 2026-07-27
Topic: stop H24 when trusted timing reaches the wrong Hard role and sparse source evidence still rejects
Phase: P023 / Controlled Expansion
Question: may H24 proceed to candidate rendering when one fresh source reaches product timing as `source_texture_bite` and the other cannot establish a guided grid?
Decision: no. Reject H24 before candidate rendering. Treat Lucid Trigger as valid continuous-texture role evidence, not as exact hit-shaper evidence or a harder-beat candidate. Treat Chopin's six detected onsets as insufficient for source-backed downbeat phase. Do not relabel texture bite, lower the `0.18`, `1.40`, or `0.30` ownership gates, tune timing from consumed H24 cases, or access additional reserve sources.
Why: Lucid proves the timing path now reaches product projection, but the frozen acceptance contract requires at least one causally owned `source_transient_chop` plus `source_hit_shaper_v3` case. Continuous distortion cannot stand in for harder drum impact. Chopin proves the tempo-guided route fails closed on sparse evidence as designed. Another unchanged holdout retry would spend freshness without clarifying the performer-owned outcome.
Evidence: Lucid matches provider 150 BPM at Rust/product 150.25041 BPM and selects `source_texture_bite`; exact callback calibration is not applicable. Chopin differs from provider 140 BPM at Rust 144.23077 BPM and reports `TempoGuided::InsufficientOnsets` with onset count six. Neither preflight emits a candidate WAV. Cave, Sector, and the bad-timing control remain untouched. Exact report hashes and the restored CC0 rotation are frozen in `docs/benchmarks/w30_resample_h24_characterization_v1.json`.
Consequences: restore Holdout A with metadata-only, unheard Black Diamond and Love Song replacements. Before H25, land a bounded development-material slice that makes the intended performer-owned Hard role and role-specific acceptance evidence explicit. `source_texture_bite` remains valid for continuous damage but cannot satisfy harder-beat proof; existing hit-shaper gates remain frozen.
Status: accepted H24 rejection; typed performer-owned Hard role gate required
...
Date: 2026-07-29
Topic: performer-owned W-30 Hard intent must constrain source-derived realization before another holdout
Phase: P023 / Controlled Expansion
Question: how should Riotbox distinguish a musician asking for harder transient/body impact from a musician asking for continuous texture damage without turning the source analyzer into the performer?
Decision: keep the existing committed `w30.apply_damage_profile` ActionCommand, but persist typed `impact` or `texture` intent with capture identity and intensity. `impact` may realize only as source-owned `source_transient_chop`; a texture-classified source reports `source_mismatch` and preserves Base. `texture` realizes as `source_texture_bite` on suitable source audio and owns no trigger grid or harder-beat claim. Missing or unsuitable source audio reports `source_unavailable`. Historical generic `Mutation` actions map explicitly to `legacy_auto`. Expose requested intent and outcome through runtime state, observer, and compact TUI. The current `D` gesture explicitly requests impact; this contract enabler does not add another audible gesture before H25.
Why: H24 reached trusted timing but selected a valid continuous texture role when the acceptance target required harder transient impact. Letting analysis silently reinterpret generic Hard made correct texture processing look like a failed harder beat and left the listener without the intended role. A typed performer request separates musical intent from source capability while preserving fail-closed source ownership and the single Action/Session/replay spine.
Evidence: serialization and replay tests preserve the typed request and legacy compatibility; queue/commit tests cover both intents; projection tests prove impact mismatch and trigger-free texture behavior; observer/TUI tests expose the result. The local RIOTBOX-1423 development matrix projects both intents across eight registered cases spanning dense, sparse, tonal, pad/noise, weak, and bad-timing material. Transient cases realize impact, the pad realizes texture and rejects impact, and unsuitable controls remain unavailable. No W-30 DSP value, hit-shaper ownership gate, H25 holdout, candidate WAV, or human verdict changed.
Consequences: RIOTBOX-1427 is a contract enabler, not sound-quality proof. RIOTBOX-1422 H25 is the directly enabled audible follow-up and may evaluate harder-beat evidence only after the committed intent is `impact` and the product outcome is `realized_impact`; texture remains a separate valid continuous-damage role.
Status: accepted typed performer intent; H25 exact live-path attempt pending
...
Date: 2026-07-29
Topic: replace W-30 Hard scalar tuning with source-aligned impact selection and fail-closed exact calibration
Phase: P023 / Controlled Expansion
Question: how should H25 use repeated musician feedback that louder, darker, dirtier, hollow, delayed, or more damaged output still does not constitute harder playable impact?
Decision: introduce versioned `source_aligned_impact_v5` for committed `impact` intent. Select real grid-near source hits from presence-attack and crest evidence, reject presence peaks more than 15 ms after the grid, require two aligned hits, preserve the dry source body, and add only a source-local nonlinear 900–3600 Hz presence residual inside the attack head. Choose the densest passing source-derived mask from full, anchored alternating, and anchored two-hit candidates. Require exact whole-window level `0.90..=1.10`, causal presence head `>= 1.10`, and crest retention `>= 0.90`; otherwise clear the product recipe and report source mismatch/unavailable without fallback audio. Keep V4 sample-stable as rejected history.
Why: earlier listening established that level, darkness, distortion, and broad waveform delta can hide body loss, late foreign attacks, unchanged beats, or listener fatigue. The intended mechanism is a source-owned transient that lands on the performance grid and remains physically distinct without replacing the source body. Cross-source rejection is part of the algorithm: an unsuitable source must not be forced into a generic Hard preset.
Evidence: five development cases span dense break, dense-full-mix stress, sparse drums, and tonal riff. Beat03 and Bertsz pass exact V5; Cinameng and Marwan correctly reject insufficient attack-over-body; Fupi fails exact level/crest and clears output. The fresh Continue holdout passes at level `1.01426`, causal head `1.14549`, and crest `1.00119`. Trance Boss Battle and Fever Stadium reject impact-role mismatch; Better Days reaches the external-120-BPM guided grid but rejects exact level `0.65606`; Anime Intro rejects an explicit 190-versus-143.084 BPM timing mismatch before projection. Independent published metadata supplies 120 BPM for tonal-riff holdout Party Sector, matching Rust `120.00001 BPM`; frozen V5 passes preflight at level `0.98332`, causal head `1.23382`, and crest `0.91443`, then the exact RuntimeMix render retains level `1.02057`, crest `0.9070`, presence attack `2.1192`, and no clipping. A search optimization based on prior development performance reuses unity render and stops impossible descending-gain work; eight accepted WAVs remain byte-identical and the rejected preflight hash remains unchanged while debug rejection time falls from roughly 180 to 30 seconds. Full evidence is in `docs/benchmarks/w30_resample_h25_characterization_v1.json`.
Consequences: H25 is a two-source/two-family technical holdout pass, not musical-alpha proof. The structured exact RuntimeMix A/B received `human_verdict: technically_ok_but_musically_weak`: Hard sounded like the same Base hit periodically retriggered or doubled, not harder. Post-listening analysis found that `99.98891%` of Base/Hard difference energy lies inside the half-second retrigger windows, while Hard has a lower true peak, `0.907` crest ratio, and `-0.4715 dB` whole-window presence-band energy versus Base. The causal-head gate compared V5 wet against dry inside the same retrigger topology and therefore did not prove harder Base-to-Hard impact. Preserve H25 as negative evidence; do not tune from consumed holdouts or relabel tonal/full-mix material as drum-only coverage. The next mechanism must avoid periodic same-onset retriggering, expose an exact wet-versus-dry V5 artifact, and prove role-appropriate Base-to-Hard attack, presence, and crest contrast on the development matrix before another holdout. The active CC0 rotation remains local, ignored, unheard, and unredistributed.
Status: technical two-family reachability accepted; musical Hard claim rejected and mechanism follow-up required
...
Date: 2026-07-31
Topic: replace repeated-onset V5 with phase-aligned multi-hit V6 and separate causal effect from Base-relative impact
Phase: P023 / Controlled Expansion
Question: how should Riotbox respond when exact V5 wet-versus-dry evidence is causal but structured listening and complete-window analysis show that Hard merely repeats one Base hit and is not harder?
Decision: preserve `source_aligned_impact_v5` sample-stably as rejected evidence and promote product selection to versioned `source_phase_aligned_impact_v6`. V6 performs each retained trigger from that slot's own analyzed source cursor and attack duration. Keep the balanced source hit only as a deterministic mask-search anchor. Record wet-over-dry causal presence head separately from Hard-over-Base presence head. Require Hard/Base head above unity and above its body ratio, retain crest at or above `0.90`, and require attack lift multiplied by retained crest above unity. Continue to fail closed without fallback audio when source role, level, causal head, Base-relative attack, or dynamics do not pass.
Why: V5's accepted `1.23382` causal ratio compared two versions of the same repeated-onset topology and therefore could not support its Base-to-Hard claim. Its reviewed Hard output had lower 900–3600 Hz and high-band energy than Base. Requiring a global crest increase alone is also not role-correct when source-derived rhythmic topology changes; a Pareto relation between Base-relative attack, Hard body, and retained crest measures the intended transient consequence without tuning a source-specific float.
Evidence: branch review found that V5's inherited 100 ms owned-hit floor flattened V6's analyzed 20–80 ms durations; V6 now retains the bounded source-derived durations while V5 stays sample-stable. Unchanged Beat03, Bertsz, and Fupi development sources pass both exact V6 preflight and the complete RuntimeMix validator; Cinameng and Marwan retain their insufficient-attack-over-body rejections. The five sources span four families. Beat03 retains six candidates and exact RuntimeMix measures Hard/Base 0–10 ms attack `2.62925`, presence head `1.66731`, body `0.94195`, and exact dry/wet presence head `1.27266`. Base, Hard, and `08_w30_aligned_impact_dry_counterfactual.wav` are distinct. No holdout was consumed. Full machine evidence is in `docs/benchmarks/w30_resample_h26_development_v1.json`.
Consequences: the exact H26 Base-to-Hard review received `human_verdict: technically_ok_but_musically_weak`. The listener found V6 very similar to Base/source and could not confidently hear Hard as harder. Post-listening analysis shows why: Hard adds only `0.896 dB` RMS and `0.181 dB` peak, crest falls from `5.200` to `4.789`, and the 20–100 ms body ratio is `0.94195` despite a `1.66731` presence-head ratio. V6 fixes repeated-onset topology and timing but yields short bright bite without enough body/dynamics for physical Hard impact. Preserve H26 as weak evidence. Before another playback, develop and cross-source-test a phase-coherent, source-adaptive body/transient mechanism; do not consume a holdout or tune loudness/presence alone. V5 artifacts remain reproducible negative evidence; rejected V6 sources remain unavailable.
Status: V6 mechanism technically accepted; H26 musical Hard claim rejected as weak
...
Date: 2026-08-01
Topic: complete W-30 Hard's source-owned impact envelope with phase-coherent body evidence
Phase: P023 / Controlled Expansion
Question: how should Riotbox respond when V6 fixes repeated-onset timing and proves a causal presence attack, but structured listening still hears only a weak bright bite because the 20–100 ms body falls below Base?
Decision: preserve V5 and V6 as reviewed negative/weak evidence and promote development selection to versioned `source_phase_coherent_body_impact_v7`. Retain V6's per-slot source onsets and adaptive attack lengths, extend the owned hit through twice the following attack duration capped at 100 ms, and derive an 80–250 Hz body center from a Hann-windowed DFT over that following body only. Initialize bounded peaking-EQ lift from `20 * log10(low_band_attack_over_body)` and allow deterministic exact search up to 12 dB. Require final 128-frame RuntimeMix whole level `0.90..=1.10`, causal wet/dry presence head `>=1.10`, Hard/Base head greater than body, body `>=1.05`, crest `>=0.95`, and level-times-crest `>=0.98`; otherwise fail closed without product output. Persist the pre-gesture W-30 grit in the typed damage action so Base calibration converges across queue, Session, restore, and replay. Keep analysis and search out of the realtime callback.
Why: H26's `1.66731` short presence-head ratio concealed a `0.94195` body ratio and reduced crest, matching the listener's report that Hard was not confidently harder. Source-centered body reinforcement completes the same physical hit without adding synthetic kicks, delayed copies, or source-specific fallback music. Persisting the real pre-gesture Base prevents the post-commit grit mutation from producing a false Hard/Base comparison.
Evidence: the unchanged five-source/four-family development matrix passes its intended mixed outcomes. Beat03, Bertsz, and Fupi pass the authoritative RuntimeMix path with body ratios `1.11442`, `1.76522`, and `1.12951`, crest ratios `0.9774`, `0.97906`, and `0.9533`, and zero clipped candidate samples. Cinameng and Marwan retain explicit insufficient-attack-over-body rejection; missing-source output remains digital silence. Direct-callback candidate search plus final product verification reduces the Beat03 debug preflight from roughly 205 to 25.84 seconds without moving analysis into realtime. Serialization, queue, restore/replay, callback-sharing, historical V3–V6 compatibility, exact calibration, and local cross-family matrix tests cover the new contract. Frozen metrics and hashes are in `docs/benchmarks/w30_resample_h27_development_v1.json`.
Consequences: no holdout has been opened and `quality_proof` remains false. The exact 7.384625-second isolated W-30 Base-to-Hard development artifact is eligible for fresh structured listening only after branch review and full CI. Bass ownership is `unassigned`; review must judge drum/transient, source-body impact, source recognition, and loop desirability, not bass pressure. A failed human verdict must preserve H27 as evidence and trigger a new causal mechanism rather than another source-specific scalar tune.
Status: V7 development mechanism accepted for structured review; musical verdict unverified
...
Date: 2026-08-02
Topic: reject phase-coherent body lift as sufficient proof of foreground Hard impact
Phase: P023 / Controlled Expansion
Question: may H27/V7 pass when exact RuntimeMix metrics prove a stronger local attack and body, but direct listening hears the result as dull, recessed, and not worth looping?
Decision: no. Preserve V7 as `technically_ok_but_musically_weak` and keep `quality_proof=false`. The next candidate must replace body-EQ-led development with a source-adaptive, onset-aligned broadband foreground-impact mechanism. It must retain V7's phase-coherent source body and fail-closed source ownership while also proving source-relative broadband peak/spectral retention, forward presence, hook contrast, and Base-relative attack/body lift across the unchanged development matrix. Low-body lift alone, darker output, loudness-only tuning, and another V7 scalar adjustment are ineligible.
Why: the listener heard V7 as slightly harder but duller and farther away, like background playback, and did not want to loop it. Exact time-local metrics explain only part of the perception: Hard/Base presence head is `1.28419`, body is `1.08281`, and crest retention is `0.97746`, but whole-segment presence and high-band energy fall `0.292 dB` and `0.988 dB`. Against the direct source, V7 is `1.285 dB` lower in RMS and `3.967 dB` lower in peak. The low-body mechanism completes the physical envelope without preserving foreground salience.
Evidence: the exact `7.384625 s` Base-to-Hard RuntimeMix artifact and exact source-to-V7 comparison were technically preflighted, bounded, played only after fresh readiness, and stopped with silence verified. The structured review records `strongest_element=chop`, `source_recognition=source_transformed_but_present`, `hook_after_two_bars=weak`, and `demo_readiness_consequence=human_weak_blocks_demo_ready_and_routes_fix`; its record hash is `112ea477d28e26b7f5e3427008646c821773cfc18dee993d3058ef507b484651`. No holdout was consumed.
Consequences: H27 cannot close RIOTBOX-1422 or become demo-ready. Preserve V5, V6, and V7 as distinct negative/weak mechanism evidence. Develop the foreground-impact mechanism on the same multi-source matrix before another human playback, then require fresh structured listening; do not spend a holdout until development listening passes.
Status: V7 musical Hard claim rejected as weak; source-adaptive foreground-impact follow-up required
