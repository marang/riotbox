...
Date: 2026-08-02
Topic: define percussive hardness as pitch-stable force rather than generic difference
Phase: P023 / Controlled Expansion
...
Consequences: retire all playback-rate transformation from the next percussive-Hard recipe. Pre-register the pitch-stable contract in `docs/engineering/percussive_hardness_contract.md`, the audio-QA spec, AGENTS guardrails, and the rave-punk production skill. A second source-synchronous attack/body attempt was mechanically rejected across three sources for near-identity and/or body loss before human playback. Product integration remains blocked until a dedicated research prerequisite establishes a defensible perceptual model, materially different algorithm candidates, and a fresh isolated human pass.
Status: accepted
