...
Decision: preserve the corrected Base byte-for-byte. For `source_transient_chop`, retrigger a tempo-derived envelope at each selected local source onset and decay it to `0.03` within `0.55` of one eighth-note step so inactive source-derived mask slots become real arrangement space. Strengthen source-reactive edge/drive for both Hard policies and apply a bounded `1.12` attack compensation only to the gated transient policy. For continuous `source_texture_bite`, retain uninterrupted playback and add bounded wavefold/quantization of the real source waveform instead of inventing a trigger grid.
...
Evidence: Beat03 Base remains byte-identical at `13d5943f0d2c222f80c279991dfbf3593b6e155b99795c916661bb3565f64a4c`. Beat03 Hard now measures `45.75%` silence, crest factor `7.789`, peak `0.709396`, and zero clips. Beat03 plus seven development/holdout cases are non-silent and unclipped, all missing-source controls are digital silence, and maximum absolute pairwise Hard-envelope correlation is `0.183690`. Seven transient policies produce relative Base-to-Hard RMS deltas from `1.062192` to `1.248608`; the continuous texture policy produces `0.152667` without grid triggers.
Consequences: waveform delta and louder output are no longer accepted as evidence of Hard articulation. Transient Hard owns attack/space contrast; texture Hard owns continuous destructive timbre. Both remain source-backed and silent when source material is unavailable. The candidate remains `human_verdict: unverified` until structured listening decides whether the new transient half finally reads as Hard and remains musically useful.
Status: proposed pending structured human listening
