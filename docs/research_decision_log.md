...
Evidence: five development cases span dense break, dense-full-mix stress, sparse drums, and tonal riff. Beat03 and Bertsz pass exact V5; Cinameng and Marwan correctly reject insufficient attack-over-body; Fupi fails exact level/crest and clears output. The fresh Continue holdout passes at level `1.01426`, causal head `1.14549`, and crest `1.00119`. Trance Boss Battle and Fever Stadium reject impact-role mismatch; Better Days reaches the external-120-BPM guided grid but rejects exact level `0.65606`. A search optimization based on prior development performance reuses unity render and stops impossible descending-gain work; eight accepted WAVs remain byte-identical and the rejected preflight hash remains unchanged while debug rejection time falls from roughly 180 to 30 seconds. Full evidence is in `docs/benchmarks/w30_resample_h25_characterization_v1.json`.
Consequences: H25 is a technical partial holdout pass, not musical-alpha proof. Only one source/family is exact-prequalified, below the required two-source/two-family minimum, so no structured human playback is authorized. Preserve V5 audio and its gates; do not tune from consumed holdouts or relabel tonal/full-mix material as drum-only coverage. The active CC0 rotation is replenished and remains local, ignored, and unredistributed.
Status: accepted V5 algorithm and fail-closed partial result; second different-family exact holdout remains open
