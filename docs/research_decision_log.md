...
Date: 2026-08-28
Topic: record one bounded Session-owned take from the real post-limiter audio callback
Phase: P016 / Pro Workflow / Live Recording
Question: what is the smallest honest live-recording path that proves Riotbox can preserve its actual running master output without substituting an offline render, blocking the realtime callback, or creating export truth outside Action and Session?
Decision: add one separately versioned `export.live_recording` action boundary, `runtime_master_capture_v1`, and one receipt boundary, `live_recording.runtime_master_capture_v1`, with pack id `live-recording-runtime-master`. The action targets the active Session scene and records exactly eight transport beats at the positive confirmed Session BPM. It captures the interleaved `f32` master samples after Riotbox's existing master limiter and immediately before CPAL device-format conversion, using the actual output sample rate and channel count. The control thread preallocates the complete capture buffer; the callback performs only bounded atomic sample stores and counter updates, with no allocation, lock, file I/O, serialization, formatting, analysis, Session mutation, or new DSP. A callback gap over 100 milliseconds, stopped transport, tempo drift, callback scratch overflow, recording-state fault, stream error, early stop, or incomplete sample count invalidates the take. Outside the callback, write one IEEE-float 32-bit WAV plus one versioned JSON proof through create-new same-directory staging files and no-clobber publication. Read both back and require exact format, frame count, sample bytes, SHA-256, active audio, post-limiter peak below full scale, host/device identity, scene and timing identity, and zero capture-window faults before committing the queued action, Session receipt, replay identity, observer completion, and host-audio ref. Refuse a missing or inconsistent confirmed BPM/scene, unsupported destination, existing destination or proof sidecar, publication/read-back mismatch, silence, clipping, or any failed gate without committing product truth.
Why: RIOTBOX-1484 gives a musician one real DAW arrangement but does not prove that the live instrument can preserve the sound produced by its actual output callback. Capturing exactly the post-limiter callback buffer is the first honest recording boundary: an offline RuntimeMix render remains useful test control but cannot stand in for host/device execution. Eight beats are long enough to preserve the current two-bar semantic hook while keeping memory, waiting time, QA, and listening bounded. Float32 preserves the callback samples without an extra quantization transform; the proof sidecar makes the external WAV independently attributable without inventing another persistence system.
Evidence: the current runtime already renders TR-909, MC-202, W-30 preview/resample, and Source Monitor into one preallocated callback scratch buffer, applies the master limiter, then converts those samples to the selected CPAL device format. `AudioRuntimeShell` already exposes actual host, device, sample rate, channel count, callback telemetry, stream errors, transport timing, and lock-free render-state updates. Core already owns the reserved `ExportLiveRecording` action shape, live-recording artifact role, Session receipt identity, `live_recording_host_audio_refs[]`, replay projection, observer lifecycle, and typed host-audio readiness blockers. This Decision was made from tracked contracts and code only; no source audio, source directory, Holdout audio, commercial reference, generated candidate, or playback was accessed.
Consequences: RIOTBOX-1485 may implement only the real runtime-master tap, bounded WAV/proof finalization, existing product-spine commit, one explicit user-session CLI/Just path, and synthetic callback/writer/failure proofs. It must not add or change sound, capture microphone/input audio, create a background recorder, support arbitrary duration or multitrack recording, enable TUI/Ghost, claim DAW host import, access Holdout/source audio, or treat offline rendering as live evidence. Human playback is required only if the exact newly recorded artifact needs a musician-facing verdict; it must follow the listening-review gate. Any tap position, duration, gap threshold, sample format, timing/scene rule, artifact shape, QA threshold, or product boundary change requires a new version and Decision.
Status: accepted
...
Date: 2026-08-29
Topic: bound runtime-master capture allocation before admitting the fixed eight-beat take
Phase: P016 / Pro Workflow / Live Recording
Question: how can `runtime_master_capture_v1` retain its frozen eight-beat/BPM rule without allowing a technically positive but pathological Session BPM or high-channel output to request an unbounded callback buffer?
Decision: retain the V1 tap position, eight-beat duration, positive confirmed Session BPM rule, sample format, and all audio/QA thresholds. Add one technical resource-admission limit of `16,777,216` interleaved samples. Reject the plan before allocation when `target_frame_count * channel_count` overflows, is zero, or exceeds that limit. The limit bounds the callback-owned atomic sample storage to 64 MiB; control-thread finalization may create one equally sized ordinary `f32` copy. Enforce the same limit in both the app plan/validation path and the audio-runtime capture boundary.
Why: code review found that the otherwise valid positive-BPM rule allowed values such as `0.1 BPM` to request hundreds of millions of atomic sample slots before any callback ran. Allocator failure or OS overcommit is not a safe admission policy for a realtime instrument. A sample-count ceiling directly bounds the actual resource without inventing a new musical minimum BPM or changing the frozen eight-beat timing contract.
Evidence: at 44.1 kHz stereo and `0.1 BPM`, the unchanged eight-beat equation requests `211,680,000` frames and `423,360,000` interleaved samples, about 1.58 GiB for the atomic buffer alone before finalization copies. The new boundary test rejects the first sample above the ceiling without allocating it, and the app test rejects the pathological positive-BPM Session before runtime capture begins. This correction used tracked code/contracts and arithmetic only; no source audio, source directory, Holdout audio, commercial reference, candidate artifact, or playback was accessed.
Consequences: ordinary V1 captures remain byte- and timing-compatible when they fit the bound. Any change to this allocation ceiling, the eight-beat duration, the BPM rule, or the callback storage representation requires a new Decision. This guard grants no new sound, source, Holdout, release, or musician-quality claim.
Status: accepted
...
Date: 2026-08-29
Topic: quantize the real runtime-master take to one exact two-bar Session window
Phase: P016 / Pro Workflow / Live Recording
Question: how can the first honest callback recorder preserve an exact musical window when V1 begins on whichever callback happens to observe the armed buffer rather than on a proved Session bar?
Decision: retain V1 as readable immutable evidence and add `export.live_recording` action boundary `runtime_master_bar_window_v2`, receipt boundary `live_recording.runtime_master_bar_window_v2`, pack id `live-recording-runtime-master-bar-window`, and proof schema `riotbox.live_recording_runtime_master_bar_window.v2`. Preserve V1's post-limiter/pre-device tap, float32 WAV, 8-beat duration, `16,777,216`-sample admission limit, writer, collision behavior, read-back gates, and Action/Session/replay/observer commit. V2 requires the exact confirmed 4/4 timing hypothesis and chooses the strictly next Session bar using that hypothesis's transport bar-grid anchor; a graphless Session uses the existing Core zero-phase 4/4 transport default. The control thread preallocates and arms the capture; pre-boundary callbacks write no samples but remain inside callback-gap telemetry. A callback that straddles the requested bar contributes only complete interleaved frames at or after the boundary. The actual first frame must fall from the requested beat through one output frame after it, and the rounded eight-beat payload may differ from the mathematical duration by at most one-half output frame. Missing the boundary, a gap anywhere in the armed/captured window, a discontinuity between captured callback windows, stopped transport, tempo drift, or any inherited runtime/writer gate fails closed. Bind the bar-grid anchor, output-frame beat span, requested/captured start and end microbeats, start-alignment and duration error in frame fractions, 4/4 geometry, armed-callback count, and timing/transport/tempo faults in the proof and typed Session host-audio timing evidence; readiness cross-checks phase and position geometry rather than trusting pass flags independently.
Why: V1 proves the real output path but its first sample is callback-scheduling dependent. Eight beats counted from an arbitrary callback edge do not prove the reusable two-bar Session window described by the product workflow. The runtime already exposes coherent render/completed transport positions for every callback, so waiting and selecting a frame offset can establish musical identity without adding another clock, blocking the callback, or changing sound.
Evidence: source-blind unit proofs cover pre-boundary waiting and gap telemetry, a straddling callback, callback-partition-identical payloads, a skipped requested boundary, and a discontinuity after capture begins. Core/Session proofs preserve V1 readiness while requiring the unique V2 alignment gate and internally consistent typed timing window; app proofs rederive the V2 plan and require exact WAV/proof/receipt identity. Branch review used the already-known Development Session metadata to expose a nonzero confirmed bar-grid anchor at cursor `3`; the rejected zero-phase implementation would have requested beat `36` after transport beat `35` instead of the exact next confirmed bar at `39`, so that artifact is retained only as failed local technical evidence and grants no claim. After correction, one real ALSA user-session run requested beat `39`, began at `39.000009` (0.182 output frame late), ended at `47.000027`, and wrote exactly `160,364` stereo float32 frames at 44.1 kHz. Its armed-plus-captured maximum callback gap was 30.188 ms, and every gap, scratch, stream, transport, tempo, timing-window, and clip fault count was zero; all three receipt gates and host-audio readiness passed. Final WAV SHA-256 is `180ef467c4f2df519a714c9225d24687889d3377f6ddf00fad00862a3c21748e`; proof SHA-256 is `efa6bfc3f5025d906e2b67ad8ac2b19b7a25c98de42a358672d68dc23452cc16`. No Holdout audio, commercial reference, new source discovery, musical decision, human listening verdict, or artifact replay occurred.
Consequences: RIOTBOX-1486 may change only callback arming/alignment, typed timing evidence, current command projection, and their product-spine/QA/docs integration. V1 remains deserializable and ready under its original rules, while the current `just live-master-recording` ingress creates V2 only. This Decision grants no sound, musical-quality, source-general, DAW-host, input-recording, arbitrary-duration, TUI/Ghost, Holdout, release, or Riotbox-completion claim. Any later change to meter support, start policy, duration, frame tolerances, tap, sample format, thresholds, artifact shape, or product boundary requires another version and Decision.
Status: accepted
...
Date: 2026-09-08
Topic: preserve external Session/Graph generations across failed saves
Phase: P000 / maintenance and regression
Question: how can ingest and save preserve the last loadable Session while retaining existing explicit external Graph paths?
Decision: keep the external path as a compatibility alias, and retain immutable canonical Graph generations beside that alias under `<graph filename>.riotbox-graphs/<sha256 hex>.json`. Before replacing an existing alias, publish its current valid Graph generation without replacement, then publish the new generation, replace the alias, and atomically replace the Session last. An existing generation is reusable only after exact canonical hash verification. The loader first validates the requested alias; on missing, unreadable, malformed, or mismatched alias it may load only the exact generation named by the Session's validated SHA-256 graph hash under that same alias. Never scan, choose a recent file, or infer another Session. Explicit graph paths keep precedence and remain constrained to their own alias/generation namespace. Session-only saves must verify that the prepared external reference already resolves before publishing it. Reject Session/Graph/generation path collisions before writes. Keep old generations without automatic garbage collection.
Reserved layout: `.riotbox-graphs` directories hold only immutable generations; mutable Graph aliases and Session destinations must not be written inside this namespace.
Why: two independent renames cannot protect a pair when the second write fails. Retaining exact hash-addressed generations allows the old Session to remain authoritative even when the compatibility alias has advanced. It also preserves existing CLI paths and does not require a new Session schema, embedded-storage migration, or a second persistence model.
...
Date: 2026-09-22
Topic: explicit numeric ownership and provisional limiter calibration status
Phase: P000 / RIOTBOX-1420 maintenance and diagnostic-evidence audit
Question: how should shared audio values retain honest provenance without coupling unrelated equal literals or retuning sound during an audit?
Decision: keep the runtime limiter knee 0.92 and ceiling 0.985 unchanged as explicitly accepted provisional engineering values; RIOTBOX-1501 owns their bounded calibration protocol. Keep zero limited samples as a categorical exact-clean-path invariant. Let exact validators consume producer-recorded thresholds, stored source/Graph format and timing, with explicit numeric-domain checks. Match the existing Rust three-way strict Fill-boundary conjunction and inclusive Alpha absolute-correlation maximum. Export the unchanged named Alpha maximum; only a missing historical V1 field adapts to the exact former f32 value, never a present invalid value. Give the twelve truly shared professional-suite thresholds one Python owner without changing their values or comparators.
Why: numerical equality is not semantic ownership. Limiter protection, waveform similarity, normalized controls and recipe gain have different units, scope and evidence. The shell previously rejected supported Fill attacks and equality accepted by Rust, imposed fixture sample rates on stored artifacts, and duplicated limiter checks. A documented but uncalibrated safety choice is more honest than an invented optimality rationale.
