...
- a local grid-locked Feral demo render pack that writes TR-909 beat/fill,
  W-30/Feral source-chop, and MC-202 bass pressure from one shared beat/bar/frame
  grid; the MC-202 lane now proves an applied source-expression plan,
  source-contour delta, bass pressure, source-grid alignment, and no fallback
  before its Development product-stem handoff may declare source-derived origin
