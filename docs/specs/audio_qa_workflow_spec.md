...
- the observer export surface derives `requested`, `started`, `completed`, and
  ...
  graph lineage when the Session has it, QA gate id/result evidence, readiness
  status, and unsupported scopes; failed records include the action id and
  failure reason. This is an observer projection, not a second export truth.
