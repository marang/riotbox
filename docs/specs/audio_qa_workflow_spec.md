...
- `just p011-exit-evidence-manifest` validates the current machine-checkable P011 evidence index across replay, recovery, export reproducibility, and stage-style stability categories; it is an evidence index, not an execution gate by itself
...
- `just stage-style-stability-smoke` / `just stage-style-stability-proof` is a CI-safe repeated-run smoke around the generated stage-style restore-diversity path; it catches obvious intermittent observer/audio collapse and full-mix nondeterminism, validates a normalized proof JSON, but remains a bounded smoke rather than a real host-audio soak or multi-hour endurance test
