...
- the observer export surface derives `requested`, `started`, `completed`, and
  ...
  hashes, per-artifact normalized manifest hash evidence, per-artifact source
  graph lineage when the Session has it, per-artifact confirmed timing-grid
  lineage when the Session has it, per-artifact audio metrics and WAV format
  evidence when the written local product artifact can be decoded safely, QA
  gate id/result evidence, readiness status, and unsupported scopes; failed
  records include the action id and failure reason. This is an observer
  projection, not a second export truth.
