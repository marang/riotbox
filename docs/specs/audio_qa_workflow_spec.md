...
`docs/engineering/percussive_hardness_contract.md`. The candidate must preserve
one onset, source pitch / `1.0x` playback, recognizable hit identity, immediate
attack, physical body, and bite. Lower pitch, louder output, darker EQ,
distortion, damage, or a passing crest/transient metric does not establish
hardness. `different_but_not_harder` is a human reject and freezes that recipe;
do not route it into another scalar-tuning pass.
