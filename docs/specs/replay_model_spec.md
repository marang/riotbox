...
- W-30 replay currently covers a deterministic cue subset whose committed actions already carry explicit target state: live recall, trigger, audition, bank swap, slice-pool browse, focus step, damage profile, hook turnaround, and pitch dive. It updates preview mode, focused bank/pad, last capture, W-30 grit where applicable, and the typed articulation profile/capture/committed start beat. Pitch Dive replay restores the persisted `pitch_dive_v1` profile; it does not rerun source selection or retune the frozen curve.
