#!/usr/bin/env python3
"""Run RIOTBOX-1444's exact four-source Development qualification fail-closed."""
...
CONTRACT = Path("docs/benchmarks/w30_pitch_dive_product_qualification_v1.json")
EXPECTED_SCHEMA = "riotbox.w30_pitch_dive_product_qualification.v1"
EXPECTED_CONTRACT_SHA256 = "1fad3f6979f40e8a6fc4f4142dfa99290a5da35524fc55f7ab0bbaa8112230da"
...
def prepare_review(
    repo: Path, contract: dict[str, Any], session: str
) -> int:
    root = repo / "artifacts/development/riotbox-1444"
    ...
    control_path = case_output / "05_w30_pitch_dive_control.wav"
    candidate_path = case_output / "06_w30_pitch_dive_candidate_v1.wav"
    ...
    review_log: dict[str, Any] = {
        "schema": "riotbox.w30_pitch_dive_product_review_access_log.v1",
        "ticket": "RIOTBOX-1444",
        "session": session,
    ...
    try:
        ...
        filter_graph = (
            "[0:a]atrim=start_sample=0:end_sample=81237,aresample=48000,"
            "aformat=sample_fmts=fltp:channel_layouts=stereo,volume=0.501187,asplit=4[s1][s2][s3][s4];"
        ...
        review_log.update(
            {
                ...
                "audible_order": [
                    "source_context_repeated_four_times_minus_6db",
                    "one_second_silence",
                    "A_exact_product_control_repeated_twice",
                    "one_second_silence",
                    "B_exact_product_pitch_dive_repeated_twice",
                ],
                "status": "passed",
...
def main() -> int:
    ...
    access_log: dict[str, Any] = {
        "schema": "riotbox.w30_pitch_dive_product_access_log.v1",
        "ticket": "RIOTBOX-1444",
    ...
    try:
        for source in sources:
            ...
            if case_id == contract["human_review"]["representative_case"]:
                command.append("--prepare-pitch-dive-review")
            completed = subprocess.run(
                command,
                cwd=repo,
                text=True,
            ...
            report_path = case_output / "pitch-dive-qualification.json"
            report = json.loads(report_path.read_text(encoding="utf-8"))
            product_bpm = float(report["exact_product_tempo_bpm"])
            require(
                abs(product_bpm - float(source["expected_product_bpm"])) <= 1.0,
                f"{case_id}: product BPM admission failed",
            )
            require(report["first_eight_beats_sample_exact"], f"{case_id}: pre-roll changed")
            require(report["silence_from_beat_twelve"], f"{case_id}: terminal was not silent")
            require(
            ...
            record.update(
                {
                    ...
                    "control_sha256": file_sha256(
                        case_output / "05_w30_pitch_dive_control.wav"
                    ),
                    "candidate_sha256": file_sha256(
                        case_output / "06_w30_pitch_dive_candidate_v1.wav"
                    ),
                    "final_four_beat_delta_rms": report["final_four_beat_delta_rms"],
                    "status": "passed",
                    "completed_at_utc": utc_now(),
                }
            )
