...
import validate_percussive_force_stage_a_v2_acquisition_batch_v2_artifacts as validator
...
def build_documents() -> tuple[
    batch_validator.FrozenAcquisitionBatchV2,
    dict[str, Any],
    dict[str, Any],
    bytes,
]:
...
def validate_log(
    frozen: batch_validator.FrozenAcquisitionBatchV2,
    log: dict[str, Any],
...
def main() -> int:
    ...
    blocked_predecessor_hashes = validator._predecessor_rejection_payload_hashes(
        frozen.document
    )
    if blocked_predecessor_hashes != set(
        validator.REJECTED_BATCH_V1_OBSERVED_PAYLOAD_SHA256
    ):
        raise AssertionError("Batch-v1 rejected payload blocklist drifted")
    ...
    cases: list[tuple[str, Callable[[dict[str, Any]], None]]] = [
        ...
        (
            "cross_version_v1_access_log_schema",
        ...
        ("batch_pin_drift", lambda value: value["contract_bindings"]["acquisition_batch_v2"].__setitem__("semantic_sha256", "0" * 64)),
        (
            "predecessor_rejection_binding_missing",
            lambda value: value["contract_bindings"].pop(
                "predecessor_acquisition_rejection_v1"
            ),
        ),
        (
            "predecessor_rejection_access_log_pin_drift",
            lambda value: value["contract_bindings"][
                "predecessor_acquisition_rejection_v1"
            ].__setitem__("access_log_raw_sha256", "0" * 64),
        ),
        (
            "predecessor_rejection_survivor_reuse_claim",
            lambda value: value["contract_bindings"][
                "predecessor_acquisition_rejection_v1"
            ].__setitem__("survivor_reuse_allowed", True),
        ),
        ...
        (
            ...
            lambda value: value["implementation_snapshot"].__setitem__(
                "repository_head_commit", "0" * 40
            ),
        ),
        ...
        (
            "cross_version_v1_access_log_path",
        ...
        (
            "cross_version_v1_manifest_name",
            lambda value: value["filesystem"].__setitem__(
                "sealed_manifest_name",
                "stage-a-v2-acquisition-sealed-manifest-v1.json",
            ),
        ),
    ...
    manifest_cases: list[tuple[str, Callable[[dict[str, Any]], None]]] = [
        ...
        (
            "cross_version_v1_manifest_schema",
            lambda value: value.__setitem__(
                "schema",
    ...
    cross_version_logs: list[
        tuple[str, str, Callable[[dict[str, Any]], None]]
    ] = [
        (
            "standalone_manifest_with_v1_access_log_schema",
            "access_log.schema",
            lambda value: value.__setitem__(
                "schema",
                "riotbox.percussive_force_stage_a_v2_acquisition_access_log.v1",
        ...
        (
            "standalone_manifest_with_v1_access_log_path",
        ...
        (
            "standalone_manifest_with_v1_batch_binding",
            "access_log.contract_bindings",
            lambda value: value["contract_bindings"].__setitem__(
                "acquisition_batch_v1",
                value["contract_bindings"].pop("acquisition_batch_v2"),
            ),
        ),
    ]
    for name, token, mutation in cross_version_logs:
        cross_log = copy.deepcopy(log)
        mutation(cross_log)
        cross_manifest = artifacts.build_sealed_manifest(cross_log)
        expect_rejected_token(
            name,
            token,
            lambda cross_log=cross_log, cross_manifest=cross_manifest: (
                validator.validate_manifest_document(
                    cross_manifest,
                    access_log=cross_log,
                    frozen_batch=frozen,
                    repo_root=REPO,
                )
            ),
    ...
    for index, predecessor_hash in enumerate(
        validator.REJECTED_BATCH_V1_OBSERVED_PAYLOAD_SHA256
    ):
        candidate = copy.deepcopy(log)
        candidate["entries"][0]["stream"]["actual_sha256"] = predecessor_hash
        expect_rejected_token(
            f"rejected_batch_v1_payload_hash_{index + 1}",
            "forbidden rejected Batch-v1 observed payload",
    ...
    print(
        f"PASS: {len(cases) + len(manifest_cases) + len(targeted) + 9} fail-closed Batch-v2 acquisition-artifact fixtures"
    )
    return 0
