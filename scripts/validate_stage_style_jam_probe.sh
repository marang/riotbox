...
cargo run -p riotbox-audio --bin w30_preview_compare -- \
  ...
  --min-rms-delta 0.001 \
  --min-sum-delta 1.0 \
  --max-active-samples-delta 400000 \
  --max-peak-delta 1.0 \
  --max-rms-delta 1.0 \
  --max-sum-delta 6000.0
...
jq -e \
  '.schema == "riotbox.observer_audio_summary.v1"
    ...
    and .control_path.first_commit == "action 1 at Immediate beat 0 bar 1 phrase 1 sequence 1"
    and .control_path.commit_count >= 7
