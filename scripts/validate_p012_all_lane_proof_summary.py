...
REQUIRED_SNIPPETS = (
    ...
    "| Path | Cue | Action | Grid source | Decision | Observer grid use | Manifest grid use | Grid compat | Downbeat compat | Downbeat ambiguity | Anchor alignment | Groove alignment | Alignment | Output issues |",
    "| cautious/manual | `needs confirm` | `confirm grid first` | `source_timing` | `source_timing_needs_review_manual_confirm` | `manual_confirm_only` | `short_loop_manual_confirm` | `compatible` | `partial` | `partial` | `partial` | `partial` | `aligned` | 0 |",
    "| user override | `needs confirm` | `confirm grid first` | `user_override` | `user_override` | `manual_confirm_only` | `short_loop_manual_confirm` | `compatible` | `partial` | `partial` | `partial` | `partial` | `aligned` | 0 |",
    "| risky override | `needs confirm` | `confirm grid first` | `user_override` | `user_override` | `manual_confirm_only` | `short_loop_manual_confirm` | `compatible` | `partial` | `partial` | `partial` | `partial` | `aligned` | 0 |",
    "| fallback | `not available` | `timing unavailable` | `static_default` | `source_timing_missing_bpm` | `unavailable` | `unavailable` | `aligned` | `partial` | `partial` | `partial` | `partial` | `aligned` | 0 |",
    "| locked grid | `grid locked` | `grid can steer moves` | `source_timing` | `source_timing_ready` | `locked_grid` | `locked_grid` | `aligned` | `aligned` | `aligned` | `aligned` | `aligned` | `aligned` | 0 |",
