#!/usr/bin/env python3
"""Run RIOTBOX-1473's exact four-source Development qualification fail-closed."""
...
CONTRACT = Path("docs/benchmarks/w30_silence_cut_product_qualification_v1.json")
EXPECTED_SCHEMA = "riotbox.w30_silence_cut_product_qualification.v1"
EXPECTED_CONTRACT_SHA256 = "925b483f91bdf87dcf4e04ec35540a5860c515877dba661ee4546bf87ca0df6c"
...
def validate_contract(contract: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    mechanism = contract.get("mechanism_contract", {})
    require(
        mechanism.get("mechanism") == "w30_choke_silence_cut_v1",
        "unexpected mechanism contract",
    )
    require(mechanism.get("mutable_after_source_access") is False, "mechanism must be frozen")
    require(
        isinstance(mechanism.get("path"), str)
        and isinstance(mechanism.get("sha256"), str)
        and len(mechanism["sha256"]) == 64,
        "mechanism path and SHA-256 must be pinned",
    )
    require(
        contract.get("product_path", {}).get("action") == "w30.silence_cut",
...
def prepare_review(repo: Path, contract: dict[str, Any], session: str) -> int:
    root = repo / "artifacts/development/riotbox-1473"
    ...
    control_path = case_output / "05_w30_silence_cut_control.wav"
    candidate_path = case_output / "06_w30_silence_cut_candidate_v1.wav"
    ...
    review_log: dict[str, Any] = {
        "schema": "riotbox.w30_silence_cut_product_review_access_log.v1",
        "ticket": "RIOTBOX-1473",
        "session": session,
    ...
    try:
        ...
        source_context_seconds = 4.0 * 60.0 / float(source["expected_product_bpm"])
        filter_graph = (
            f"[0:a]atrim=duration={source_context_seconds:.9f},"
            "aresample=48000,aformat=sample_fmts=fltp:channel_layouts=stereo,"
            "volume=0.501187,asplit=2[s1][s2];"
            "[s1][s2]acrossfade=d=0.02:c1=tri:c2=tri[src];"
...
def main() -> int:
    ...
    require(
        file_sha256(repo / mechanism["path"]) == mechanism["sha256"],
        "frozen Development mechanism contract SHA-256 changed",
    )
    ...
    access_log: dict[str, Any] = {
        "schema": "riotbox.w30_silence_cut_product_access_log.v1",
        "ticket": "RIOTBOX-1473",
    ...
    try:
        for source in sources:
            ...
            if case_id == contract["human_review"]["representative_case"]:
                command.append("--prepare-silence-cut-review")
            completed = subprocess.run(
                command,
                cwd=repo,
                text=True,
            ...
            report_path = case_output / "silence-cut-qualification.json"
            report = json.loads(report_path.read_text(encoding="utf-8"))
            require(
                report.get("schema") == "riotbox.w30_silence_cut_qualification_case.v1",
                f"{case_id}: unexpected qualification report schema",
            )
            require(
                report.get("mechanism") == "w30_choke_silence_cut_v1",
                f"{case_id}: unexpected mechanism",
            )
            require(report.get("committed_start_beat") == 8, f"{case_id}: wrong start beat")
            require(
                report.get("isolated_contributors") == ["w30_preview"],
                f"{case_id}: isolated contributor drift",
            )
            require(report.get("fade_milliseconds") == 5.0, f"{case_id}: fade timing drift")
            require(report.get("silence_cut_beats") == 1.0, f"{case_id}: cut length drift")
            ...
            for key in (
                "prefix_before_fade_sample_exact",
                "one_beat_cut_pcm_zero",
                "sample_exact_after_five_ms_return",
                "boundary_delta_rule_passed",
            ...
            require(
                float(report["effect_window_delta_rms"])
                > float(contract["technical_gates"]["minimum_effect_window_delta_rms_exclusive"]),
                f"{case_id}: effect delta gate failed",
            )
            require(
                int(report["control_cut_window_active_samples"])
                > int(contract["technical_gates"]["minimum_control_cut_window_active_samples_exclusive"]),
                f"{case_id}: silence has no active control counterfactual",
            )
            ...
            record.update(
                {
                    ...
                    "control_sha256": file_sha256(case_output / "05_w30_silence_cut_control.wav"),
                    "candidate_sha256": file_sha256(
                        case_output / "06_w30_silence_cut_candidate_v1.wav"
                    ),
                    "effect_window_delta_rms": report["effect_window_delta_rms"],
                    "control_cut_window_active_samples": report[
                        "control_cut_window_active_samples"
                    ],
                    "status": "passed",
                    "completed_at_utc": utc_now(),
                }
