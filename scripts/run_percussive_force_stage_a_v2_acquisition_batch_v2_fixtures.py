#!/usr/bin/env python3
"""Synthetic no-network execution fixtures for the RIOTBOX-1430 Batch-v2 runner."""
...
import percussive_force_stage_a_v2_acquisition_artifacts as historical_artifacts
import run_percussive_force_stage_a_v2_acquisition as historical_runner
import run_percussive_force_stage_a_v2_acquisition_batch_v2 as runner
import validate_percussive_force_stage_a_v2_acquisition_batch_v2_artifacts as artifact_validator
...
EXPECTED_REQUIRED_HEAD_BLOB_FILES = (
    "Justfile",
    "docs/benchmarks/percussive_force_stage_a_protocol_v1.json",
    "docs/benchmarks/percussive_force_stage_a_protocol_v2.json",
    "docs/benchmarks/percussive_force_development_matrix_v1.json",
...
def prepare_repo(destination: Path) -> None:
    ...
    run_fixture_git(destination, "init", "--quiet", "--template=", "--initial-branch=fixture")
    run_fixture_git(destination, "add", "--force", "--", ".")
    commit_fixture_repo(destination, "fixture baseline")


def run_fixture_git(repo: Path, *arguments: str) -> bytes:
    ...
    if completed.returncode != 0:
        raise AssertionError(
            f"fixture Git command failed: {' '.join(arguments[:2])}: "
            f"{completed.stderr[:512]!r}"
        )
    return completed.stdout


def commit_fixture_repo(repo: Path, message: str) -> None:
    run_fixture_git(
        repo,
        "-c",
        "user.name=Riotbox Fixture",
        "-c",
        "user.email=fixture@riotbox.invalid",
        "-c",
...
def create_historical_v1_rejected_log(repo: Path) -> bytes:
    batch = json.loads(
        (REPO / "docs/benchmarks/percussive_force_stage_a_v2_acquisition_batch_v1.json").read_text()
    )
    first = batch["entries"][0]
    from urllib.parse import urlsplit

    expected_target = urlsplit(first["download_url"]).path
    calls: list[str] = []
    dns_calls = 0
    original_resolve = historical_runner._resolve_one_public_endpoint
    original_open = historical_runner._open_exact_https_response

    def resolve(_host: str) -> tuple[list[str], historical_runner.ResolvedEndpoint]:
        nonlocal dns_calls
        dns_calls += 1
        return ["1.1.1.1"], historical_runner.ResolvedEndpoint(
            historical_runner.socket.AF_INET,
            ("1.1.1.1", 443),
            "1.1.1.1",
        )

    def reject_first(
        _endpoint: historical_runner.ResolvedEndpoint, request_target: str
    ) -> FakeSession:
        calls.append(request_target)
        assert request_target == expected_target
        return FakeSession(
            FakeResponse(
                b"",
                status=302,
                declared_length=first["attachment_byte_count"],
            )
        )

    historical_runner._resolve_one_public_endpoint = resolve
    historical_runner._open_exact_https_response = reject_first
    try:
        expect_failure(lambda: historical_runner.run_acquisition(repo))
    finally:
        historical_runner._resolve_one_public_endpoint = original_resolve
        historical_runner._open_exact_https_response = original_open
    assert calls == [expected_target]
    assert dns_calls == 1
    historical_log = repo / HISTORICAL_V1_ACCESS_LOG_PATH
    payload = historical_log.read_bytes()
    document = json.loads(payload)
    assert document["schema"] == historical_artifacts.ACCESS_LOG_SCHEMA
    assert document["attempt_status"] == "rejected"
...
def run_historical_v1_reject_log_is_irrelevant_fixture() -> None:
    with tempfile.TemporaryDirectory(
        prefix="riotbox-1430-batch-v2-v1-log-irrelevant-"
    ) as temporary:
        repo = Path(temporary)
        prepare_repo(repo)
        historical_log = repo / HISTORICAL_V1_ACCESS_LOG_PATH
        historical_log.parent.mkdir(parents=True, exist_ok=True)
        historical_payload = create_historical_v1_rejected_log(repo)
        before = historical_log.stat()
        fixture = NetworkFixture()
        with PatchedNetwork(fixture):
            runner.run_acquisition(repo)
        after = historical_log.stat()
        assert fixture.calls == [1, 2, 3]
        assert fixture.dns_calls == 3
        assert historical_log.read_bytes() == historical_payload
...
def run_historical_v1_schema_reconcile_rejection_fixture() -> None:
    with tempfile.TemporaryDirectory(
        prefix="riotbox-1430-batch-v2-v1-schema-reconcile-"
    ) as temporary:
        repo = Path(temporary)
        prepare_repo(repo)
        v2_log = repo / batch_contract.ACCESS_LOG_PATH
        v2_log.parent.mkdir(parents=True, exist_ok=True)
        historical_payload = create_historical_v1_rejected_log(repo)
        v2_log.write_bytes(historical_payload)
        v2_log.chmod(0o600)
        fixture = NetworkFixture()
        with PatchedNetwork(fixture):
            try:
                ...
                assert "not the frozen Batch-v2 schema" in str(error)
            else:
                raise AssertionError("v1 access-log schema reconciled as Batch-v2")
        assert fixture.calls == []
        assert fixture.dns_calls == 0
        assert v2_log.read_bytes() == historical_payload
        assert not exact_exists(repo, batch_contract.QUARANTINE_DIRECTORY)
        assert not exact_exists(repo, batch_contract.FINAL_BATCH_DIRECTORY)


def run_required_head_runner_blob_rejection_fixture(mode: str) -> None:
    with tempfile.TemporaryDirectory(
        prefix=f"riotbox-1430-batch-v2-head-runner-{mode}-"
    ) as temporary:
        repo = Path(temporary)
        prepare_repo(repo)
        runner_relative = "scripts/run_percussive_force_stage_a_v2_acquisition_batch_v2.py"
        runner_path = repo / runner_relative
        if mode == "missing":
            run_fixture_git(repo, "rm", "--quiet", "--cached", "--", runner_relative)
            commit_fixture_repo(repo, "remove runner blob from HEAD")
            expected_error = f"required HEAD blob is missing: {runner_relative}"
        elif mode == "mismatch":
            runner_path.write_bytes(runner_path.read_bytes() + b"\n")
            expected_error = (
                f"worktree bytes differ from required HEAD blob: {runner_relative}"
            )
        else:
            raise AssertionError(f"unknown required-HEAD runner mode: {mode}")
        ...
        with PatchedNetwork(fixture):
            try:
                ...
                raise AssertionError(
                    f"required HEAD runner-blob {mode} fixture failed open"
                )
        assert fixture.calls == []
        assert fixture.dns_calls == 0
...
def run_forbidden_batch_v1_survivor_fixture(forbidden_digest: str) -> None:
    with tempfile.TemporaryDirectory(
        prefix="riotbox-1430-batch-v2-forbidden-survivor-"
    ) as temporary:
        repo = Path(temporary)
        prepare_repo(repo)
        fixture = NetworkFixture()
        original_stream = runner.acquisition.stream_exact_attachment

        def collide_with_v1(*args: Any, **kwargs: Any) -> dict[str, Any]:
            record = original_stream(*args, **kwargs)
            return {**record, "actual_sha256": forbidden_digest}

        runner.acquisition.stream_exact_attachment = collide_with_v1
        try:
            with PatchedNetwork(fixture):
                try:
                    runner.run_acquisition(repo)
                    ...
                    assert (
                        str(error)
                        == "payload_identity_collision_forbidden_batch_v1_survivor"
                    )
                else:
                    raise AssertionError("Batch-v1 payload survivor was accepted")
        finally:
            runner.acquisition.stream_exact_attachment = original_stream
        assert fixture.calls == [1]
        assert fixture.dns_calls == 1
        assert not exact_exists(repo, batch_contract.FINAL_BATCH_DIRECTORY)
        assert not exact_exists(repo, batch_contract.QUARANTINE_DIRECTORY)
        log = read_log(repo)
        assert log["attempt_status"] == "rejected"
        assert log["successful_request_count"] == 0
        assert log["entries"][0]["state"] == "response_metadata_verified"
        assert all(entry["state"] == "not_requested" for entry in log["entries"][1:])
...
def run_implementation_drift_fixture() -> None:
    with tempfile.TemporaryDirectory(prefix="riotbox-1430-drift-") as temporary:
        ...
        def close_callback(ordinal: int) -> Callable[[], None] | None:
            ...
            def drift() -> None:
                path = repo / "scripts/run_percussive_force_stage_a_v2_acquisition_batch_v2.py"
...
def run_repository_head_drift_fixture() -> None:
    with tempfile.TemporaryDirectory(prefix="riotbox-1430-head-drift-") as temporary:
        repo = Path(temporary)
        prepare_repo(repo)
        ...
        def close_callback(ordinal: int) -> Callable[[], None] | None:
            ...
            def drift() -> None:
                (repo / ".git/refs/heads/fixture").write_text("2" * 40 + "\n")
...
def main() -> int:
    assert artifacts.REQUIRED_HEAD_BLOB_FILES == EXPECTED_REQUIRED_HEAD_BLOB_FILES
    assert len(artifacts.REQUIRED_HEAD_BLOB_FILES) == len(
        set(artifacts.REQUIRED_HEAD_BLOB_FILES)
    )
    frozen_forbidden_hashes = frozenset(
        batch_contract.FORBIDDEN_OBSERVED_PAYLOAD_SHA256
    )
    assert len(frozen_forbidden_hashes) == 2
    assert frozen_forbidden_hashes == runner.FORBIDDEN_BATCH_V1_PAYLOAD_HASHES
    assert {
        runner.PUBLICATION_PROBE_SOURCE_NAME,
        runner.PUBLICATION_PROBE_DESTINATION_NAME,
    }.isdisjoint(
        {
            historical_runner.PUBLICATION_PROBE_SOURCE_NAME,
            historical_runner.PUBLICATION_PROBE_DESTINATION_NAME,
        }
    )
    assert {
        batch_contract.ACCESS_LOG_PATH,
        batch_contract.QUARANTINE_DIRECTORY,
        batch_contract.FINAL_BATCH_DIRECTORY,
        batch_contract.SEALED_MANIFEST_NAME,
    }.isdisjoint(
        {
            historical_runner.batch_contract.ACCESS_LOG_PATH,
            historical_runner.batch_contract.QUARANTINE_DIRECTORY,
            historical_runner.batch_contract.FINAL_BATCH_DIRECTORY,
            historical_runner.batch_contract.SEALED_MANIFEST_NAME,
        }
    )
    run_success_fixture()
    run_historical_v1_reject_log_is_irrelevant_fixture()
    ...
    run_historical_v1_schema_reconcile_rejection_fixture()
    run_required_head_runner_blob_rejection_fixture("missing")
    run_required_head_runner_blob_rejection_fixture("mismatch")
    for forbidden_digest in sorted(frozen_forbidden_hashes):
        run_forbidden_batch_v1_survivor_fixture(forbidden_digest)
    run_failure_fixture(
    ...
    run_repository_head_drift_fixture()
    ...
    print("PASS: 50 no-network Batch-v2 one-shot runner fixtures")
    return 0
